...
同じ観測で各`wsl.exe`の親を最大8段まで辿り、容量回収・SSH・エディタ・shell・
PowerShellなど固定した分類だけを集計する。実際のプロセス名・ID・パス・引数は出さない。
親が消えた場合、循環、子より後に作られた親は観測不能とし、未知の名前は`other`で止める。
WSL再起動元を絞る材料であり、プロセスの本人性や停止権限の証明には使わない。
親の分類が観測不能でもworkerの存在確認・完了判定は変えない。
...
main の repository/native 検証とmanual `real-git-push-e2e` は互換性を追加する。
Seed専用のprivate-registry手動jobは製品経路とともに撤去した。過去の受入を現行の永続Storeの
レジストリ互換性の証拠へ読み替えず、その確認は未完了として残す。`windows-wsl-image-cache` は download cache で、受入ではない。
