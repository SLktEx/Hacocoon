...
- **Implemented dedicated trusted networking:** the Incus adapter owns and validates `haco-host0`; fresh hosts inherit no profiles and the installer no longer infers network initialization from storage. A bounded current-host NIC transition preserves the root disk/UUID. Docker coexistence is limited to the owned bridge and established replies. Installer DNS/route/HTTPS checks, ownership/migration regressions and an isolated Linux packet gate are implemented; final packaged acceptance is pending. See [ADR 0005](adr/0005-trusted-host-network-ownership.md).
- **Final packaged candidate `3f67845` (2026-09-06):** cached BAT application and same-version rerun both exited 0. Cold ordinary WSL entry, controller protocol 1, product version in both execution domains, explicit rejection of the removed `haco host` bridge, normal shell/process exit 0 and retained file checksum passed. Trusted-host UUID, account state and every sudo-policy hash matched before/after rerun. Fresh creation was tested on `57b6ee2`, not repeated destructively for this final candidate.
...
- **Implemented cache performance correction:** suppress PS5.1 per-chunk progress locally during image download. Component tests exercise actual cache promotion/reuse, hash-mismatch cleanup and caller preference retention; Windows acceptance still uses the unmodified packaged BAT.
- **Observed Windows acceptance (2026-09-06):** after `9d459be` failed at explicit elevation, `57b6ee2` invoked system WSL in the original console and completed the unmodified cached BAT with exit 0. Entry, Btrfs rootfs, controller communication and same-version BAT completion passed. Normal shell exit later proved broken, as detailed below; restart/rerun interrupted the waiting sessions. Trusted-host identity/file and all sudo-policy hashes were retained; no product overrides or preparation fixtures were used.
...
- **Prior packaged network acceptance (`3f67845`):** installer-created `incusbr0` supplied trusted-host DNS/routes. HTTPS timed out on `57b6ee2` while Docker FORWARD policy was DROP; on packaged `3f67845`, Physical Host and trusted-host HTTPS returned 200, including trusted-host after same-version rerun. Read-only rules then showed Docker FORWARD policy ACCEPT. No firewall repair was applied; the cause of the policy change and coexistence across startup orders remain unverified. The Windows gate requires both layers. That candidate still created an unused directory pool; the current correction removes minimal initialization, with final fresh acceptance pending.
- **Interactive-exit correction:** final observation of `57b6ee2` found that shell `exit` printed logout but did not return the WSL process; restart/rerun later interrupted the waiting connection. Entry and file retention passed, but normal exit did not. The adapter now separates process completion from open client input, and the login alias rejects non-terminal character devices. Focused process regressions cover open-input success/nonzero exit. Packaged `8a44f17` later passed cold WSL entry, controller ping, retained-file checksum and actual WSL process exit 0 after shell `exit`.
- **WSL startup correction:** `5b3d36d` completed packaged BAT application, but a subsequent ordinary login ran before the controller socket existed. Read-only inspection then found the enabled controller starting normally about 19 seconds into the WSL boot. Login now waits for a bounded, read-only readiness ping without creating another controller. Startup, rejection and timeout regressions pass. Packaged `8a44f17` passed entry after explicitly terminating only Hacocoon, without a preliminary root/service probe, and returned process exit 0. Account, trusted-host UUID and all sudo-policy hashes match the earlier installation.
- **Pending real-host acceptance:** repeatable trusted-host networking across startup orders, allowed proxy versus denied direct Environment traffic, SSH and Environment/Workspace work retention. The complete updated Windows gate has not passed. Linux Incus/network foundation CI continues.
- **Next concrete acceptance:** validate the dedicated-network change through the packaged cached BAT and current-data retention; Environment deny/proxy, SSH and lifecycle product flow remain separate follow-ups.
