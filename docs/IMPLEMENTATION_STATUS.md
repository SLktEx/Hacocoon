...
| [Cache generations](design/cache-generations.md) | partial | Atomic source selection, detached Incus volume copies and canonical Env-owned disposable-data lifecycle are implemented on this candidate. Linux rootfs/managed-repository placement and exact manual/client resume are implemented candidates; repository placement requires the native custom-volume file API. Host configuration/target selection, collection, history, clearing, recovery of unknown creation and full cache/large-repo acceptance remain incomplete. |
