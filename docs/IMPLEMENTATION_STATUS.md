...
| [Cache generations](design/cache-generations.md) | partial | Atomic whole-source selection and detached Incus cache-volume copies are implemented on this candidate. Host configuration, normal-Env path collection/placement, disposable attachments, history, clearing and real cache/large-repo acceptance remain planned. |
