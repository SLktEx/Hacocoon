...
- **Partial product CLI:** new `haco` provides help/version, `doctor`, and the controller-backed WSL login alias. It never delegates to `hacoq`. Startup waits for the existing controller; interactive process exit does not wait for stdin closure. The installer's explicit `hacoq host ensure` dependency and the older lifecycle/Base/SSH surface remain migration work. The #456 controller adapter remains reusable.
...
- **Next concrete implementation:** replace the installer's explicit `hacoq host ensure` dependency with controller-backed bootstrap.
