...
- **Pending WSL acceptance:** one-shot BAT installation, installer-created network DNS/routes/HTTPS, allowed proxy versus denied direct Environment traffic, restart and current-installer rerun with data retention. Repository tests and the Linux Incus fixture do not establish these user-path results. Current Windows CI still uses first-launch setup and a second BAT invocation.
- **Next dependency:** rebase the useful one-shot installer work from #441 onto the reset CLI and resolve #452 without a temporary broad sudo grant. #455/#456 remains the reusable controller/SSH follow-up; do not route product commands through `hacoq`.
