...
The first source-cleanup GHA candidate failed during fixture instance creation: its image was selected from the Hacocoon project but the isolated project attempted a default-project image lookup. The fixture now explicitly selects the source image project through Incus native creation. Later dependent Base/OCI checks in that failed job were skipped, not passed; the corrected dedicated WSL native fixture passed in 28.05s, including public CLI and owned cleanup. Latest-candidate GHA validation remains pending.
