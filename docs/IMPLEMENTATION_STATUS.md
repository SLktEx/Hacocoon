...
- **Observed Windows acceptance (2026-09-06):** after `9d459be` failed at explicit elevation, `57b6ee2` invoked system WSL in the original console and completed the unmodified cached BAT with exit 0. Ordinary entry, Btrfs rootfs, controller communication, restart before reinstall, and same-version BAT rerun passed. Trusted-host identity/file and all sudo-policy hashes were retained; no product overrides or preparation fixtures were used.
...
- **Network blocker:** Physical Host HTTPS passed; installer-created `incusbr0` supplied trusted-host DNS/routes, but trusted-host HTTPS timed out. Docker-managed IPv4 FORWARD DROP rules coexist with Incus accept rules. The Windows gate now requires DNS/routes/HTTPS from both layers. `incus admin init --minimal` also created an unused directory pool; single-pool bootstrap is not complete.
- **Interactive-exit correction:** final observation of `57b6ee2` found that shell `exit` printed logout but did not return the WSL process; restart/rerun later interrupted the waiting connection. Entry and file retention passed, but normal exit did not. The adapter now separates process completion from open client input, and the login alias rejects non-terminal character devices. Focused process regressions cover open-input success/nonzero exit; rebuilt packaged acceptance is pending.
- **WSL startup correction:** `5b3d36d` completed packaged BAT application, but a subsequent ordinary login ran before the controller socket existed. Read-only inspection then found the enabled controller starting normally about 19 seconds into the WSL boot. Login now waits for a bounded, read-only readiness ping without creating another controller. Startup, rejection and timeout regressions are implemented; final packaged entry/exit acceptance remains pending.
- **Pending real-host acceptance:** trusted-host HTTPS, allowed proxy versus denied direct Environment traffic, SSH and Environment/Workspace work retention. The complete updated Windows gate has not passed. Linux Incus/network foundation CI continues.
