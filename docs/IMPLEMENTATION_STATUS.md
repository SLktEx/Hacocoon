...
The current milestone position is **v0.70**. See [versioning and release status](status/versioning-and-release-status.md) for numbering authority and history.
...
| [Cache generations](design/cache-generations.md) | partial | Atomic source selection, detached Incus volume copies and canonical Env-owned disposable-data lifecycle are implemented on this candidate. Incus placement/collection, Host configuration, history, clearing, recovery of unknown creation and real cache/large-repo acceptance remain incomplete. |
