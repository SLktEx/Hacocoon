...
- **Observed packaged acceptance — `b71f88e`:** the unmodified `install-windows.bat -UseCachedWslImage` completed current-install application and same-version rerun with exit 0. Ordinary and explicitly stopped/restarted WSL entry, guest `haco setup`, both clients' five-check doctor reports and complete controller/client build identities passed. The retained file, trusted UUID, UID/GID, locked password, all sudo-policy hashes, Btrfs placement and live mount policy matched the baseline. One standalone cold `wsl --exec haco doctor` ran before controller startup; the CLI now waits through bounded read-only ping, with focused race/regression coverage. Packaged acceptance of that later wait is pending. An intermittent WSL user-session warning remains unexplained; a later explicit cold entry passed without it.
...
- **Commit-bound hosted acceptance:** [Windows exact BAT/restart/rerun](https://github.com/SLktEx/Hacocoon/actions/runs/33999277059) passed on `a4c6e2d`, including fresh creation and trusted-host sentinel retention. Product source is unchanged from `b71f88e`; two harness fixes separately corrected native negative-test exit propagation and ConPTY carriage-return matching, with negative regressions. [Ubuntu installer](https://github.com/SLktEx/Hacocoon/actions/runs/33997745295) and [all four Incus jobs](https://github.com/SLktEx/Hacocoon/actions/runs/33997744487), including the updated setup/client-only lifecycle gate, passed on `b71f88e`. These results do not accept later product changes.
...
- **Next concrete implementation:** complete M1 layer diagnostics with short causes/next actions, then wire and verify the installed Standard Environment proxy with denied direct traffic. Reboot continuation and final candidate acceptance remain required.
