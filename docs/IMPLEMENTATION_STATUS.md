...
The current milestone position is **v0.25**. Milestones are lightweight development checkpoints: v0.17 still has acceptance work, but that partial status does not block later implemented checkpoints such as v0.18-v0.25.
...
Repository tests do not substitute for all real-host acceptance. v0.23 now proves a phased real-Incus substrate plus Core lifecycle on GitHub-hosted Ubuntu 26.04, and v0.25 additionally proves ordinary-user managed-Btrfs CLI behavior through the privileged helper boundary. Real Incus networking/resource behavior beyond those paths—including proxy-only bridge ACL/dnsmasq behavior—Windows/WSL + VS Code, private-registry credentials, Docker compatibility, physical managed-Btrfs compression/COW/compaction behavior, broader storage failure injection, desktop notification delivery, and future cloud adapters remain environment-dependent. Partial acceptance in an earlier milestone does not prevent later minor checkpoints from advancing.
