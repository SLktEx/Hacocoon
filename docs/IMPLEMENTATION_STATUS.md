...
- **Implemented in this repository slice:** Incus is the only local Btrfs lifecycle owner. The former storage implementation and privileged executable are absent from code, installer payloads and CI. Mount policy is `compress=zstd:3,noatime,nodiscard`; existing pools reconcile through Incus with readback and no write on an exact match. See the [storage contract](design/btrfs-storage-layout.md).
- **Partial product CLI:** `haco` currently exposes help/version and the controller-backed WSL login alias. The retained lifecycle, Base, SSH and OCI commands described below belong to temporary `hacoq`; they are not implemented new `haco` workflows. See [CLI migration](CLI_MIGRATION.md).
- **Planned Seed retirement:** Seed code remains, including optional Base revision resolution. The [Seed design](design/oci-seed-and-cow.md) records the dependency separation; Base and optional Plugin contracts remain.
- **Implemented installer continuation:** the first BAT completes normal Ubuntu OS dialogs and continues into root-side common preparation with the validated ordinary UID/GID. No installer sudo policy is written. Current-installation reruns preserve the configured user. PS5.1 argument transfer has regression coverage. See [ADR 0004](adr/0004-wsl-installer-authority.md).
- **Pending WSL acceptance:** full one-shot installation, installer-created network DNS/routes/HTTPS, allowed proxy versus denied direct Environment traffic, restart and current-installer rerun with data retention. The previous storage candidate's real BAT created WSL and requested another run; that is a reproduced failure, not acceptance. Windows CI now drives OS dialogs and ordinary entry without the old root-user/sudoers fixtures; this changed gate has not yet run on the candidate. Its trusted-host file check does not prove Environment/Workspace work retention.
- **Next dependency:** verify the installer-created network and remove storage-presence inference from network initialization. #455/#456 remains the reusable controller/SSH follow-up; do not route product commands through `hacoq`.
...
| Managed Btrfs transparent compression | default Incus pool creation requests `compress=zstd:3,noatime,nodiscard`; `compress-force` and `autodefrag` are not desired defaults, while filesystem mount lifecycle remains Incus-owned | v0.21 implemented |
