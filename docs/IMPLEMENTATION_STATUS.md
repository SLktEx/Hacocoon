...
E5 OCI image list/delete now support reviewed --unused candidates (including tagged images without container users). Existing per-image ownership, reference and absence checks remain. Native batch acceptance is pending; cache/other-resource GC remains planned. See [image selection](design/oci-image-deletion.md#review-unused-image-candidates).
