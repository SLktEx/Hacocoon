...
- **Implemented controller diagnostics:** `haco doctor [--json]` checks runtime, configured Btrfs storage, owned running host/network and fixed trusted connectivity through a bounded read-only API. It neither repairs nor creates guest-local state. Failed/skipped or malformed responses cannot succeed. Release client/controller build identities must match in packaged acceptance. See [diagnostic contract](design/controller-client-transport.md#host-diagnostics).
- **Observed packaged acceptance — `7798b57`:** the unmodified `install-windows.bat -UseCachedWslImage` completed current-install application with exit 0. Ordinary WSL entry, text/JSON doctor and actual shell/process exit 0 passed. Both ordinary Physical Host and trusted-host clients returned all five checks OK and the exact controller version/commit/build date. The retained file, trusted UUID, UID/GID, locked password state, all sudo-policy hashes, Btrfs placement and live mount options matched the baseline. The same-version BAT rerun also exited 0, followed by explicit termination of only Hacocoon and successful ordinary entry without root/service preflight. Doctor, retained state and actual process exit 0 passed again.
- **Repository validation:** full Go tests/vet and focused race checks passed on the Go source in `63fdf24`, unchanged in `7798b57`; PS5.1 installer components, five Windows diagnostic assertions, docs checks, installer packages, workflow policy and GoReleaser build/checksums passed. The maintained `ci-local.sh test` invocation stopped before Go because Ubuntu lacked Node; equivalent Go and native Windows Node checks passed separately.
- **Pending real-host acceptance:** fresh creation of `7798b57`, Windows reboot, networking across firewall reload/startup orders, allowed proxy versus denied direct Environment traffic, SSH and Environment/Workspace work retention. The complete updated Windows gate has not passed. Firewall observations changed from Docker FORWARD DROP to ACCEPT with no DOCKER-USER chain; no acceptance-time repair was applied, and the cause/order remains unverified. The separate isolated Linux DROP packet gate is not live Windows Docker or Environment proxy acceptance.
...
- **Setup repository validation:** maintained `ci-local.sh test` passed the full shuffled Go suite, vet, two JavaScript syntax checks and five notification tests. The validator used Go 1.27.1 in Ubuntu and the existing Windows Node runtime via a test-toolchain wrapper. Focused race tests and installer stage-order regressions passed. Product environment and installed resources were not modified by those tests.
- **Implemented controller-owned setup:** the installer uses `haco setup`; the legacy bootstrap path and guest hacoq provisioner are removed. Tests cover fixed validated companion sources, partial client provisioning, retained ownership, concurrent/lost-client requests, safe failures and installer stage ordering. New packaged acceptance is pending; the `7798b57` results above predate this replacement. See [ADR 0006](adr/0006-controller-owned-host-setup.md).
- **Next concrete implementation:** complete M1 layer diagnostics with short causes/next actions, then verify Environment proxy/direct behavior and a fresh candidate ZIP without deleting existing user data.
