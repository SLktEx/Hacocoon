...
| [Legacy OCI Seed / Docker](reference/cli-migration.md) | partial | Optional `HACO_PLUGIN_OCI=nerdctl` or `docker` integration remains on temporary `hacoq`. Seed command/composition/resolver entry points are removed on this candidate; builder/harvest/telemetry internals and old-data migration remain. Existing Seed data is retained. It is not the current persistent Store workflow. |
