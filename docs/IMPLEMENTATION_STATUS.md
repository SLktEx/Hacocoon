...
| [Repository / Workspace](guides/git-workflow.md) | implemented | Clone an existing branch; create independent managed copies and collections. Exclusive leases survive stop. Selected membership in independent forks and checkout/linked-worktree input are implemented; in-place membership editing is unsupported. General interrupted Workspace-preparation recovery remains incomplete; interrupted source registration can be discarded with `haco repo delete -f`. |
...
| [Retained-object cleanup](guides/data-lifetime.md) | implemented | Reviewed Workspace, built Base and whole Store deletion retain their normal protection checks. Source deletion attempts cleanup after confirmation despite Workspace references, incomplete state or native children; `-f/--force` also skips source review/confirmation. Exact managed native targeting and positive final absence are still required before the source record is released. |
