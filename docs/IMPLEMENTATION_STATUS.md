...
The current milestone position is **v0.72**. See [versioning and release status](status/versioning-and-release-status.md) for numbering authority and history.
...
|---|---|---|
| [Standard Host tools](design/trusted-host.md#standard-host-tools) | implemented | Normal local setup supplies Git/gh and pinned containerd/nerdctl/BuildKit before user recipes, using managed OCI data and Host-local sockets. Repeat setup preserves data. Duplicate preliminary Git/gh installation is removed; the canonical Host preparation owns all package/tool stages. Installed acceptance of this setup change remains pending. Released Windows installer, arm64 runtime and custom existing-installation acceptance remain separate. |
| [Daily entry / setup diagnostics](reference/daily-workflow.md) | implemented | Bounded setup stages and correlation IDs on stderr, final-frame validation and exclusion through disconnect; noninteractive confirmations do not wait. Notification refresh preserves fixed service-operation failures. Dedicated Linux acceptance does not establish Windows default-entry/IDE acceptance. |
...
| [Cache generations](design/cache-generations.md) | partial | Atomic source selection, detached Incus volume copies and canonical Env-owned disposable-data lifecycle are implemented on this candidate. Linux rootfs/managed-repository placement and exact manual/client resume are implemented candidates; repository placement requires the native custom-volume file API. The Standard Host target/scope/compatibility selector is implemented but not registered; public configuration, collection, history, clearing, recovery of unknown creation and full cache/large-repo acceptance remain incomplete. |
...
| [Legacy OCI Seed / Docker](reference/cli-migration.md) | partial | Optional `HACO_PLUGIN_OCI=nerdctl` or `docker` integration remains on temporary `hacoq`. Seed command/composition/resolver entry points and the harvest runner are removed on this candidate; managed-Environment markers remain in identity code. Builder/maintenance/Seed catalog internals are also removed. OCI sampling/recommendation and shared deletion-state separation, plus old-data migration, remain partial. Existing Seed data is retained. It is not the current persistent Store workflow. |
