...
- **Observed cache regression (2026-09-06):** packaged `831e5f0` BAT failed before distro creation because `Get-FileHash` was unavailable. Restored #459's module-independent SHA-256 helper lost during #441 integration, with a PS5.1 regression. Fresh-install acceptance must be rerun on the corrected package.
...
- **Observed Windows boundary (2026-09-06):** verified cached-image candidate `9d459be` failed WSL creation with exit code 1 after UAC, before a Hacocoon distro or Incus existed. The correction invokes system WSL in the original console and lets WSL own prerequisite elevation; progress/errors remain visible and a failure stops before common setup. Packaged BAT acceptance of the correction is pending.
