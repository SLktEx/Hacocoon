...
Seed observation/deletion remains under `haco plugin oci ...`. The physical v0.18 builder/publisher must also remain outside Core behind the OCI/provider adapter boundary; it must not turn containerd, nerdctl, OCI manifests, Incus images, or Btrfs subvolumes into Core vocabulary.
...
- v0.18 performs the actual immutable Seed build/publish/COW lifecycle.
