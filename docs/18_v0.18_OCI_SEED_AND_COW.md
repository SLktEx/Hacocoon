...
Seed observation/deletion/build/current remain under `haco plugin oci ...`. The physical v0.18 builder/publisher stays outside Core behind the OCI/provider adapter boundary; it does not turn containerd, nerdctl, OCI manifests, Incus images, or Btrfs subvolumes into Core vocabulary.
...
The first repository slice does not make v0.18 complete. Remaining work includes:
...
- v0.18 owns the actual immutable Seed build/publish/COW lifecycle.
