...
| v0.10 | VS Code Remote Agent Host Adapter | ✅ 実装済み。real host acceptanceはpending |
| v0.11 | Base Images & Custom Environments | ✅ first slice実装済み。build/import/history/GCはfollow-up |
| v0.12 | Sandbox Resource Limits | ✅ first slice実装済み。real workload enforcementはhost-dependent |
| v0.13 | Optional OCI Plugin | ✅ first slice実装。opt-in nerdctl/Docker inventory + Seed recommendation foundation |
...
**実装済みのproduct progressionはv0.13 optional-plugin first sliceまで連続**している。

次の作業はOCIをCore必須にすることではなく、optional plugin境界のままSeed build/publishとreal-host acceptanceを完成させること。
...
v0.1 ───────────────────────────── v0.12
                        ...
                        optional OCI plugin first slice
                              ...
                              /       |       \
                             v        v        v
                      Seed build   promotion  deletion
                       v0.13A       v0.13B     v0.13C
```

OCI specificationが存在しても、OCI toolingがCore機能になるわけではない。

`HACO_PLUGIN_OCI`未設定なら、Hacocoonは`containerd` / `nerdctl` / Docker CLI / Docker Engineを要求しない。
...
project-maintainedな`containerd + nerdctl`構成はoptional profile。本物のDocker CLI / Docker Engine compatibilityもoptional。

OCI toolingが一切ないHacocoon Core installationもvalidとする。

## 番号変更の履歴

2026-08-30の整理で、design-onlyだったBase関連を既に実装済みだったper-agent workより前に置く一時的な番号割り当てを解消した。
...
v0.13をmandatory Local Registryとして書いた古い資料や、`containerd + nerdctl`をHacocoon runtimeそのものとして書いた記述は、`00A_PLUGIN_ARCHITECTURE.md` / `OCI_RUNTIME_AND_DOCKER_COMPAT.md` のplugin境界で置き換える。

古いcommit message、closed PR、candidate branch、過去のplanning textに旧表現が残っていてもhistorical recordとして扱う。
...
- **v0.13:** plugin境界・driver selection・telemetry・recommendationはrepository実装済み。real OCI profile/container-tool acceptanceはpending
- **v0.13A:** Seed build/publishとreal Btrfs/COW acceptanceはplanned
- **v0.13B:** selection policyは実装済み。future Seed Builderでの実際のconsumeはplanned
