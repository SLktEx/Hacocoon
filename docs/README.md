...
This file defines how to read Hacocoon documentation after the 2026-08-29 architecture rebaseline, the v0.8 implementation progression, the v0.9 Base-image roadmap contract, the v0.10 per-agent sandbox broker, and the v0.11 VS Code Remote Agent Host Adapter.
...
- v0.10 implements trusted persisted session-to-Environment binding outside Core.
- v0.11 implements `haco-agent-host`, a thin remote-SSH adapter for the VS Code Agents window; it does not implement AHP.
- `IMPLEMENTATION_STATUS.md` is the source for current repository reality.
- real-provider/client acceptance remains separate from unit, integration, fake-provider E2E, race, vet, build, release, and CI checks.
...
- v0.10 can contain a session broker while real Agent Host/AHP routing remains pending.
- v0.11 can prepare the standard SSH target while real current-VS-Code Agents-window/Agent-Host behavior still requires external acceptance.
...
v0.9 remains implementation-pending until code actually lands.

## v0.10 reading path

1. `10_v0.10_PER_AGENT_SANDBOX_AND_AGENT_HOST.md`
2. `10_v0.10_PER_AGENT_SANDBOX_AND_AGENT_HOST.ja.md`
3. `02_v0.2_WORKSPACE_ABSTRACTION_AND_LEASE.md`
4. `08_v0.8_CLIENT_ADAPTERS_AND_VSCODE_INTEGRATION.md`
5. `09_v0.9_BASE_IMAGES_AND_CUSTOM_ENVIRONMENTS.md`
...
1. `11_v0.11_VSCODE_REMOTE_AGENT_HOST_ADAPTER.md` — authoritative v0.11 contract.
2. `11_v0.11_VSCODE_REMOTE_AGENT_HOST_ADAPTER.ja.md` — maintained Japanese explanation.
3. `10_v0.10_PER_AGENT_SANDBOX_AND_AGENT_HOST.md` — persisted ownership underneath the adapter.
4. `CLIENT_ACCESS.md` — existing loopback SSH boundary.
5. `08_v0.8_CLIENT_ADAPTERS_AND_VSCODE_INTEGRATION.md` — Windows/WSL client configuration boundary.
6. `IMPLEMENTATION_STATUS.md` — implementation versus real-client acceptance.

```text
haco-agent-host prepare --session <id> <worktree>
        |
 v0.10 persisted binding
        |
 loopback SSH alias
        |
...
The v0.11 isolation unit is the prepared Hacocoon session slot, not an automatically discovered VS Code-internal session UUID. Several VS Code sessions intentionally created through one prepared alias may share its Environment.
