...
The common phase now checks daemon readiness without minimal initialization. The adapter explicitly owns the Btrfs pool and trusted-host bridge; fresh installs do not create an unused default directory pool. Existing pools are preserved. Current owned default-profile hosts receive a bounded NIC transition without data deletion. See [trusted networking](design/trusted-host.md#dedicated-trusted-host-network) and [ADR 0005](adr/0005-trusted-host-network-ownership.md). Current-install packaged acceptance passed; fresh acceptance remains pending.
