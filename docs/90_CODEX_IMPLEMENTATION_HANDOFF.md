...
Fully implemented product milestones are contiguous through **v0.16**. v0.17 Docker Compatibility Plugin has a foundation but is incomplete. v0.18 Optional Local OCI Registry and v0.19 OCI Seed Builder & Btrfs/COW are planned.
...
When a new independent feature lands, its implementation PR must take the next `v0.N` and update both numbering and implementation-status docs. Security fixes, bug fixes, hardening, refactors, CI, docs, release engineering, and test-only work normally do not consume another product version.
...
Base name/revision             -> provider-neutral Environment contract
...
Read `13_v0.13_MANAGED_SANDBOX_NETWORK.md` before changing the managed Incus network/profile/ACL path.

Rules:

- use Hacocoon-owned managed network/profile state;
- verify before use and fail closed on drift;
- never silently fall back to broad/default Incus networking;
- do not pretend IP/CIDR ACLs provide domain-name authorization;
- domain-aware egress policy belongs in a higher-layer broker/proxy/plugin.

### v0.14 — Git Fetch Plugin

Read `14_v0.14_GIT_FETCH_PLUGIN.md` and the Git/GitHub capability docs.

Rules:

- keep fetch under `haco plugin git`;
- keep reusable GitHub credentials on the trusted Host;
- use the trusted `gh auth git-credential` path;
- reject repository-controlled credential/transport rewrites;
- do not move Git into Core.

### v0.15 — OCI Seed Recommendation

Read `15_v0.15_OCI_SEED_RECOMMENDATION.md`.

Rules:

- telemetry stores OCI identity, not arbitrary Environment/workspace data;
- recommendations require immutable digests;
- repeated samples from one Environment do not inflate weight;
- deterministic top-10% automatic promotion selects content only, never credentials;
- selection does not claim physical Seed publication exists.

### v0.16 — OCI Image Deletion

Read `16_v0.16_OCI_IMAGE_DELETION.md`.

Rules:

- deletion identity is reference + immutable digest;
- mutable ambiguity/tag movement fails closed;
- no forced all-Environment deletion;
- tombstones override automatic Seed re-promotion;
- published immutable Seeds are never edited in place;
- partial destructive work becomes recovery-required.

### v0.17 — Docker Compatibility Plugin

Read `17_v0.17_DOCKER_COMPATIBILITY_PLUGIN.md` and `OCI_RUNTIME_AND_DOCKER_COMPAT.md`.

Rules:

- standard runtime remains containerd + nerdctl;
- Docker compatibility stays optional and outside Core;
- genuine Docker CLI compatibility is allowed;
- `dockerd` should be on-demand rather than a permanent Hacocoon requirement;
- avoid duplicate image stores/layers without a demonstrated need;
- current repository foundation is not yet the complete v0.17 gate.

### v0.18 — Optional Local OCI Registry

Read `18_v0.18_LOCAL_OCI_REGISTRY.md` before implementation.

A registry/proxy is optional. Do not make it a prerequisite for normal `nerdctl pull` or for OCI Seed. Mandatory mediation, when explicitly configured, must fail closed rather than silently bypassing policy.

### v0.19 — OCI Seed Builder & Btrfs/COW

Read `19_v0.19_OCI_SEED_AND_COW.md` before implementation.

Never:

- share one writable `/var/lib/containerd` between Environments;
- copy live containerd internals as a Seed-building shortcut;
- manipulate hidden Incus/Btrfs state behind the provider;
- open unrestricted Internet egress for the Seed Builder;
- embed reusable Host registry credentials into a Seed.

Seed identity binds immutable parent Base revision + immutable OCI digests. Acquisition happens on the trusted Host; the builder stays offline; COW comes from normal Incus/storage-driver cloning.
...
- v0.11 Bases resolve mutable names once to immutable revisions before Environment creation.
- v0.12 requested finite resource limits fail closed if the provider cannot enforce/read them back.
- v0.7 EC2 remains experimental/default-off.
...
Individual Go/docs checks remain documented in `CONTRIBUTING.md`.

Real Incus, networking/resource enforcement, Base/image lifecycle, Windows/WSL + VS Code, Agent Host/AHP, private registries, and AWS/EC2/SSM/EBS require suitable external environments before being reported as passed.
