...
リポジトリ棚卸しでは通常の`bindings/*.json`も読み、保存されたEnv名・Workspaceと
各メンバー・リポジトリの参照を`bindings`へ表示します。通信先URL・ブランチ・設定・
資格情報の内容は出しません。単一レポと複数レポは既存の参照抽出を共用します。
同じリンク非追跡・読取り中の変化確認に加え、brokerと同じ1ファイル16 KiB、
ディレクトリ全体で4,096項目の上限を使います。不正ファイル・名前不一致・リンク・
未知の項目があっても、正常に読めた記録は残します。保存済みの関連付けであり、
現在有効な接続の証明ではありません。`state_validated`と`authority`はfalseのままです。
復元後の接続は通常の製品コマンドで作り、この記録を権限として採用しません。
