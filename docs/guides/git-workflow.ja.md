...
取得元リポジトリの削除は明示的な後始末操作で、Workspaceの記録が接続先として使っている途中でも実行できます。独立したWorkspaceファイルは残りますが、Git仲介の再接続には同じ取得元の再登録が必要になる場合があります。中断した取得元登録を一覧確認・対話確認なしで破棄するには `haco repo delete -f <id>` を使います。
