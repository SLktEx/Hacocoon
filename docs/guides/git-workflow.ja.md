...
`haco doctor <env>`で環境のGit接続を診断します。Git接続の失敗が確認された場合は、
`haco doctor --fix <env>`で修復します。リモートへのfetch・pushや承認ルールの変更は行いません。
Gitを使わないWorkspaceは対象外で、停止中・状態不明の場合は修復しません。
従来の利用者向け`haco git connect`コマンドは削除しました。
...
pushはmainだけを毎回確認します。main限定の読み取りルールは全ブランチfetchを許可しません。cloneではpush許可を保存しません。
...
この項目のないルールは拒否されます。開発ブランチを確認付きでpushするには、その正確な
...
pushが失敗・中断したら、信頼された管理側でEnv名を指定します。
...
`status`は最後に要求したpushの保存記録を読み、リモートには接続しません。
`reconcile`は対象ブランチの読み取りについて新しくPolicy・承認を確認し、現在位置を照合します。
提案したcommitとの一致、送信前のcommitとの一致、ブランチ不存在、別commitへの移動を表示します。
一致だけでは誰が変更したかを断定できず、元のpushが未確認なら未確認のままです。
共通Capabilityの完了とGitの送信確認は、JSONでも別の事実として保持します。
どちらの操作もpushの再送や、中断した承認の復元は行いません。

識別子・OIDを読む場合は`--json`、過去の要求を選ぶ場合は`--request <request-id>`をEnv名より前に
指定してください。実行中・承認待ちなら終了を待ちます。送信前記録の欠落、Env世代・取得元所有者の
変更、現在の接続の欠落、監査破損は照合を拒否します。別の対象を推測せず保持記録を確認してください。
送信所有権を記録していない旧版の要求は、このコマンドで照合できません。

その後Env内でfetchし、リモートとの差分を見直してから、必要なら通常のpushを新しく行います。
新しいPolicy判断とリモートの正確な状態確認が必要です。mainのpush承認は維持します。
開発ブランチの実装であり、新しい処理の外部認証Git・導入済みクライアントでの受入は未確認です。
[記録と照合の契約](../adr/0086-git-push-reconciliation-evidence.md)も参照してください。

## 取得・コミット・push

Environmentでは通常の `git status`、`git fetch origin`、`git pull --ff-only`、
`git add`、`git commit`、`git push` を使います。
1回のbatchで最大1024個のSHA-1ブランチを取得できます。各ブランチのpack上限は16 GiBです。
転送全体をメモリに保持せず少しずつ受け渡し、一つを取り込んでから次を取得します。
batchの合計を32 MiBに制限しません。
fetchは既存のローカルブランチ履歴を再利用し、既存ブランチへのpushは、
一覧にある更新前の履歴が手元にある場合に再送を省きます。新規ブランチでも、一覧にある
利用可能な祖先を新しい読み取り確認後に再利用します。対象のheadが移動したり読み取りを
拒否された場合は、エラーを確認してfetchしてからやり直してください。pushはまだ送信
されていません。利用できる祖先がなければ全体packが必要です。中断・不完全な転送は
失敗するため、エラーを確認してからやり直してください。
