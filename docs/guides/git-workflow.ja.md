...
1回のbatchで最大1024個のSHA-1ブランチを取得できます。packは各ブランチで32 MiBまでで、
一つずつ取り込んでから次を取得するため、batchの合計は32 MiBを超えても構いません。
...
されていません。利用できる祖先がなければ全体packが必要です。新しいpack自体が
一つのブランチのpack自体が32 MiBを超える転送には引き続き対応していません。
