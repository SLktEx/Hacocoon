...
現在のmilestone位置は **v0.70**。番号の正本と履歴は[バージョンとリリース状況](status/versioning-and-release-status.ja.md)を参照してください。
...
| [キャッシュ世代管理](design/cache-generations.ja.md) | partial | 共通元の排他的な採用、未接続Incus領域のコピー、Env用使い捨て領域の共通ライフサイクルを開発候補に実装。Incusへの配置・収集、Host設定、履歴・クリア、結果不明の作成の復旧、実キャッシュ・巨大レポの受入は未完了です。 |
