...
| [通知・クライアントAPI](reference/interaction-events.ja.md) | 実装済み | 情報を絞ったeventと任意のadapter。VS Codeの確認と回答は既存管理APIを使うローカルGUI内で完結。Windowsレビューは限定範囲で確認済み。新規GUI・人のトースト回答・Linux起動は未確認。 |
