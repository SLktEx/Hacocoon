...
| [通常のGit操作](guides/git-workflow.ja.md) | 部分実装 | 全head取得と各refの独立した読み取り確認、新規ブランチ一つまたは既存fast-forwardの内容固定push承認。clone/fetchはpush許可を与えず、mainへの承認を維持。送信済みpushは正確なold/newとリモート観測を照合し、自動再送しない。packは32 MiBまでで、LFS/submodule、force・削除・複数ref、不明結果からの一般的な復旧は制限または非対応。認証付き実機利用と巨大レポは別途受入が必要。 |
