...
- **次の具体的実装:** 伸ばしたcold起動待ちを受入確認し、既存Standard proxyのlifecycleをinstall済みPhysical Host controllerへ接続する。残るM1の層別/live-storage診断と再起動時の続行を完成させ、Environment直接通信の禁止を維持する。
