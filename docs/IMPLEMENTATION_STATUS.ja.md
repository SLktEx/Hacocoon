...
現在のmilestone位置は **v0.71**。番号の正本と履歴は[バージョンとリリース状況](status/versioning-and-release-status.ja.md)を参照してください。
...
| [キャッシュ世代管理](design/cache-generations.ja.md) | partial | 共通元の排他的な採用、未接続Incus領域のコピー、Env用使い捨て領域の共通ライフサイクルを開発候補に実装。LinuxのEnv本体への配置と、所有関係を照合する手動・自動再開を実装候補に追加。レポ相対配置・収集、Host設定、履歴・クリア、結果不明の作成の復旧、キャッシュ全体・巨大レポの受入は未完了です。 |
