...
現在のmilestone位置は **v0.72**。番号の正本と履歴は[バージョンとリリース状況](status/versioning-and-release-status.ja.md)を参照してください。
...
| 機能 | 状態 | 使える範囲・制約・残課題 |
|---|---|---|
| [Host の標準ツール](design/trusted-host.ja.md#host-の標準ツール) | 実装済み | 通常のローカル setup がユーザースクリプトの前に Git/gh と固定版 containerd/nerdctl/BuildKit を導入。管理対象 OCI データと Host 内のソケットを利用し、再 setup はデータを保持。重複する先行Git/gh導入を撤去し、正規Host準備がパッケージ/ツール段階を所有。この変更の導入済み受入は確認待ち。公開版 Windows インストーラー、arm64 実機、独自の既存導入環境の確認は別途必要。 |
| [日常操作・setup診断](reference/daily-workflow.ja.md) | 実装済み | 制限付きの進捗・相関IDをstderrへ出力し、最終応答を検証。切断後も処理終了まで排他を保持し、非対話の確認は入力待ちしない。通知更新は失敗したサービス操作を固定の理由で保持。専用Linuxでの検証とWindows既定エントリー・IDEの確認は別。 |
...
| [キャッシュ世代管理](design/cache-generations.ja.md) | partial | 共通元の排他的な採用、未接続Incus領域のコピー、Env用使い捨て領域の共通ライフサイクルを開発候補に実装。LinuxのEnv本体・管理対象レポへの配置と、所有関係を照合する手動・自動再開を実装候補に追加。レポ配置にはIncusのcustom volumeファイルAPIが必要です。StandardのHost対象・共有範囲・互換条件の選択は実装済みですが製品へ未登録です。公開設定、収集、履歴・クリア、結果不明の作成の復旧、キャッシュ全体・巨大レポの受入は未完了です。 |
...
| [Baseの作成](design/base-images-and-custom-environments.md) | 部分実装 | 定義からのビルド、論理ID・revisionの参照、確認付きイメージ削除。[PackerのHCL2・外部shell構築](design/packer-base-builds.ja.md)はこの候補で部分実装。Env内実行・上限付きファイル転送を実装し、導入済み経路の受入は未完了。Baseは初期rootfsの選択と由来を表し、スナップショットが保持する実体の依存先ではない。 |
...
| [旧OCI Seed・Docker](reference/cli-migration.md) | 部分実装 | 任意の`HACO_PLUGIN_OCI=nerdctl`または`docker`連携は移行用`hacoq`に残る。この候補でSeedのコマンド・composition・resolver入口とharvest runnerを撤去。管理対象Envのマーカーは既存の識別処理へ移した。builder・保守・Seedカタログ内部も撤去。OCI収集/推奨と共有削除記録の分離、旧データ移行はpartialで、既存Seedデータは保持。現行の永続Store手順とは別。 |
...
公開Windows待受・インストーラへの組み込みと通常WSL入口からの自動委譲は実装候補です。
導入済みWindows/WSL/Incus経路の受入は残件です。転送開始案内は日英文を一度だけ
