...
現在のmilestone位置は **v0.59**。番号の正本と履歴は[バージョンとリリース状況](status/versioning-and-release-status.ja.md)を参照してください。
...
| [通常のGit操作](guides/git-workflow.ja.md) | 部分実装 | 全heads fetch/pull（1024 heads・pack合計32 MiB）と、controller所有の資格情報による内容固定push。単一ref fast-forward push承認は読み取りと分離。大きなpack、ブランチ作成・削除、force push、LFS/submodule、不明な結果からの一般的な復旧は非対応。全headsの実Env受入は未確認。 |
