...
**実装済み候補:** 既存の読み取り一覧を使う保守用の走査と持ち運べる一覧の比較で、
内容・mode・数値所有者・ツリー内リンク・通常ファイル/ディレクトリのxattrを照合します。
差分と観測未完了を区別し、環境全体の復元・認証付きの開発再開は部分実装のままです。
[操作手順](guides/data-evacuation.ja.md#復元したツリーを照合する)を参照してください。

## 容量回収の言語表示

**実装済み候補:** 通常の容量回収・結果確認、保持記録と容量表示を共通の日英カタログへ
揃えました。別に記録した導入環境の登録情報の見え方の差を解決したことにはなりません。
