...
- **Standard proxy lifecycleはimplemented:** install済みunitは既存Physical Host controllerのStandard proxyを有効にし、nftablesを明示的に導入する。bind前に共有guardを検証し、control/proxyの終了を連動させ、停止時にはhijack済みCONNECTも閉じる。daemonはambient approval providerを持たず、exact allowはauditを維持し、require-approvalはfail closedとなる。維持されるlocal Go全suite/vet/通知client検証、対象race、実socket cancel/半切断、Policy、logging、Windows assertion 9件、installer実shell 5件、文書整合性が成功した。`abd4125` の [Windows正規gate](https://github.com/SLktEx/Hacocoon/actions/runs/34004464659)、[Ubuntu installer](https://github.com/SLktEx/Hacocoon/actions/runs/34004467228)、[Incus全4job](https://github.com/SLktEx/Hacocoon/actions/runs/34004466315) が成功した。Windows gateは固定listenerのPIDがcontrollerと同じこと、未管理Physical Host sourceをHTTP 403で拒否することも検証する。実機BAT適用・同版再実行は終了0。適用後の通常入口・両client・保持基準・同じproxy観測も成功した。service所有権と拒否の証拠であり、Environment許可通信の証明ではない。[ADR 0007](adr/0007-controller-owned-standard-egress.ja.md) を参照。
- **guest起動待ちのpackage受入 — `42e2fb38ea3a18a479c811bf547dd4f609858542`:** [fresh Windows BAT/停止/再実行/cold gate](https://github.com/SLktEx/Hacocoon/actions/runs/34006145146) は成功した。実機ZIPのSHA-256は `ded1ce6c250bb0d2ec1681ebf8420faf022eb183dcf987688d59921a50e8ae74`（build `2026-09-06T02:18:24Z`）。正規cached BATの適用・同版再実行は終了0。停止確認後の直接doctorは34.605秒で全5項目が成功した。その後の通常入口では、11:33:30 JSTにIncus本体PID 282がSIGKILLを受け、標準unitの600秒start-post待機とcontroller依存が残って失敗した。signal送信元は未確定で、得られたkernel記録はOOMを示していない。手動service/mount修復なしで11:43:16にIncus標準の自動再起動が始まり、後の通常入口・両client識別・保持基準・同PIDのproxy listener・未管理sourceのHTTP 403は成功した。後の成功で起動失敗を取り消さず、cold/通常起動の安定性は未解決とする。
- **live storage診断はimplemented、package受入pending:** 読み取り専用adapterは設定検査と別に `storage_mount` を追加し、backing device/inode・単一の全image loop関連付け・実Btrfs root mountを照合する。検証済みpolicy不一致は `pending` と終了1で示し、不明・不正・観測中の変化はfail closedとする。mount lifecycleや修復は追加しない。対象race/vet、pendingのCLI/API回帰、Windows assertion 9件は成功した。[storage所有文書](design/btrfs-storage-layout.ja.md#読み取り専用のmount診断)を参照。
- **次の具体的実装:** live-storage診断packageの受入とIncus起動時SIGKILLの送信元特定。M1の層別診断・再起動時の続行・install済みEnvironment proxy制御を進め、M2/M3のCLI全面移行は分ける。
