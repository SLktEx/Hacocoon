...
- Run distro creation in the initiating Windows user's console using the system `wsl.exe`. WSL owns prerequisite elevation and console attachment. Do not unconditionally elevate registration with `Start-Process -Verb RunAs`: it obscures native diagnostics in a short-lived second console and unnecessarily changes process authority. Preserve native output separately from the exit-status decision; any failure stops before common setup.
- Run the common Ubuntu installer through `wsl --user root --exec`, under the initiating Windows user's existing WSL authority.
