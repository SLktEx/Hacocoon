...
Status: accepted; implementation candidate. [日本語](0091-client-stream-forwarding.ja.md)
...
Native Windows listener transport through `wsl.exe` is an [implemented candidate](0092-wsl-process-transport.md).
Installed acceptance, remaining process consolidation and VPN/DNS modes remain separate M3 work. Native Linux listeners remain local; ordinary WSL entry delegates to Windows as specified by [automatic delegation](0093-windows-tunnel-delegation.md). See the [owning contract](../design/controller-client-transport.md#client-tcp-listeners).
