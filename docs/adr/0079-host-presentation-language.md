# ADR 0079: Pass a normalized Host presentation language
...
before opening the Host session, unless `HACO_UI_LANGUAGE`, `LC_ALL` or
`LC_MESSAGES` is explicitly set.
...
entry notice and travels through the existing validated session request.
Noninteractive login and native Ubuntu never run this query. Explicit
`haco host shell` uses the same interactive Host entry selection.
