# ADR 0084: Bind ephemeral cleanup to its creation
...
Catalog schema 14 requires every run's exact creation identity. Missing or
malformed identities are refused without adopting a current same-name Env or
rewriting its ownership. Old-version readers/migration and fallback cleanup are
outside the user's current scope. Existing data is not automatically deleted.
...
catalogs, refusal without adoption, retained data and bounded cancellation.
Repository checks and existing captured-run Incus acceptance are separate from
native acceptance of this change. Interactive stdin/TTY uses the same ownership path; see [ADR 0085](0085-bounded-process-streams.md).
