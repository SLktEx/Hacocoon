...
状態: 採用・実装候補。[English](0091-client-stream-forwarding.md)
...
Windows側待受から`wsl.exe`を通すtransportは[実装候補](0092-wsl-process-transport.ja.md)です。
導入済み経路の確認、汎用process streamの残る統合、VPN／DNS modeはM3の別残件です。通常Linuxの待受は手元に置き、通常のWSL入口は[自動委譲](0093-windows-tunnel-delegation.ja.md)に従ってWindowsへ転送します。
