...
状態: accepted、部分的な実装候補。[English](0073-wsl-process-transport.md)

## 決定

Windows clientは検証した同一PCのdistributionを選び、非表示のSystem32 wsl.exeから
導入済み製品の固定操作を呼びます。通知とcontroller転送で引数・環境変数の構成を
共有します。任意コマンド・WSL利用者変更・継承認証環境・任意socketは受け取りません。
...
公開companionの配布と通常WSL入口からの自動委譲は実装候補です。後者は[ADR 0074](0074-windows-tunnel-delegation.ja.md)を参照してください。
