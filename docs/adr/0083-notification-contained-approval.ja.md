...
COM生成・呼び出しの診断は固定段階と数値HRESULTだけを保持します。読み取り専用の起動の期限切れ・中止は個別の拒否コードで区別し、利用不能を要求なしへ読み替えたり回答を再送したりしません。


## 起動準備の確認

WSLの非公開peerが読み取り専用listへ応答してから、COMの表示受付を登録します。
既存の起動通知待ちと同じ20秒を、所有履歴clear・peer準備・最初のreview全体で共有します。
プロセス作成だけを準備完了と扱いません。通常の表示・読取の10/8秒上限は呼出期限内で維持し、準備完了で権限を与えたり、要求を再送したり、queue投入を表示成功にしたりしません。
COM登録を子プロセス・履歴のcleanupより先に解除し、終了中の表示loopへ重複起動を受け付けません。
