...
開発実装 `fe23cf539a8f27e56d8357354540c9a7f8cc5488`
（[#703](https://github.com/SLktEx/Hacocoon/issues/703)）はLinuxの対象選択7回帰に成功した。
Gitメタデータ・変更中のファイル・symlinkを持つ実ツリー二組をコピーして照合し、
その後の差分と未復元の対象をそれぞれ区別した。既存の走査・照合を再利用し元データを保持した。
Windowsは移植可能な一覧処理6件に成功し、Linuxファイルシステム用1件は明示的にSKIPした。

Linuxのコマンドは対象選択0.24秒、既存ツリー照合0.25秒、共通のローカル全体test84.69秒、
docs18.25秒、workflow policy1.98秒が成功した。開始前にWSLのroot用systemd user sessionの
警告が出たが、サービス修復や権限変更で通してはいない。最後の日英文書は `check_docs.py` に成功。
選択したローカルファイルとリポジトリの証拠であり、導入済みIncus、実際の対象の網羅性、
...
実装 `f79744c38445021f5498e70ed87956669e3345da` は、CLI・controller・正規のBase処理へ
任意のbuilder名を渡す。通常Envの名前検証を共通化し、同名の試行も新しい一時Workspaceを作る。
作成失敗では削除せず、不確定な公開や失敗したcleanupでは対象名を保持する。
通信規則の変更や承認は行わない。

ローカルはCore/Base/workspace/controller/CLI/Packerの対象13.09秒、変更lint19.97秒、
全体test65.21秒、関連race18.46秒、CLI E2E5.68秒、docs11.01秒、workflow policy1.68秒、
native試験コンパイル1.84秒が成功した。#704を変更せず取り込んだ後も、対象選択0.11秒、
比較0.13秒、統合全体test76.30秒、docs10.65秒、workflow policy1.43秒が成功した。
同じcommitから通常のLinux/Windowsインストーラ候補10バイナリを35.38秒で生成した。
生成した `haco base build --help` も日英で成功し、builder名指定の案内を確認した。
導入・リリースはしていない。WSLのroot user session警告は残し、サービスや権限を修復していない。

リポジトリ・ローカルファイル・配布物生成の証拠である。以前の通常Packer依存取得HTTP403は、
範囲を絞って確認した設定と実際の取得・build・公開・再利用が通るまで未解決のまま残す。
自動審査で拒否された全Env向け規則は未適用であり、名前指定もその例外にはしない。
本人の承認・ログインはリリース後の確認として残す。

2026-09-16に、[Packer 1.16.0の公式配布](https://releases.hashicorp.com/packer/1.16.0/)のチェックサム一覧とZIPの中央ディレクトリ情報を
上限付きで読み、`prepare.py` の固定チェックサム2件との一致を確認した。
amd64のアーカイブ/実行ファイルは34,785,580 / 108,318,882 bytes、arm64は
31,509,969 / 100,663,458 bytesで、既存の128 MiB上限内だった。
配布情報だけの確認であり、実行ファイル全体の取得・実行はしていない。
以前のguest依存取得HTTP403を、この結果で解決済みにはしない。
専用ローカルWSLでは別のGo処理が動いており、C:の空きは約11 GBだった。
読み取りだけに留め、導入内容・サービス・Policyを変更せず、新たなWSLの追加・再起動・
広い通信規則での受入試験も行っていない。

### Windows切り離し拒否の再発とプロセス観測

以下の後続Windows公開容量回収は、Linux両段階と停止要求の成功後に
`compact_attached`で失敗した。各359回のnative確認後も圧縮は未開始。
通常導入・HTTPS・SSH/エディタ/転送・独立したLinux回収は成功し、後続通知はSKIPだった。

| 同一head | Windows run / job | 同じ対象の再開 |
|---|---|---|
| `9a8a7d07f93677690c04cb6124c8d78bb5373965`（#704、載せ替え前） | 35006335176 / 104506895607 | 失敗 |
| `4c94462ffd6e42d5f1e831cff41224fe051021ba`（#704） | 35008285127 / 104513489162 | 成功 |
| `c0692cdf327be7f809333c2a9ea23753bd1f5661`（#705、文書更新前） | 35009099013 / 104516251265 | 成功 |
| `428b2bdb53b1070ec4d870bf7589778f29f90042`（#705） | 35011032772 / 104522727488 | 成功 |
| `38aeae56ed769b41e59c0d9e89859f2f7e50c52e`（#706、個数のみ） | 35012950293 / 104529173573 | 成功 |
| `c3a2cc8678cdfc4e396d6ac0699eb1cca9d0c626`（#706、2回目） | 35016859540 / 104544769661 | 成功 |

以前の実測回収成功は当時の範囲で保持し、これらの後続失敗は#381で未解決として追う。
virtual観測ハンドルは各回で閉じ、公開driverはworker完了前にWSLへ再入せず、
専用readerもConPTYの読み取りを続けている。現行記録だけではLinux停止中・WSL切り離し待ち・
別の再起動を区別できない。

実装 `356b8c509107aa0f37eec5ce04aefe274b604e44` は、公開試験のworker観測に
Windows側CIMのプロセス数を追加する。変化した個数・経過時間・固定した範囲と状態だけを
記録する。個数は見えるWSL全体のもので、対象所有情報やディスク切り離しの証明ではない。
製品の操作・期限・権限・再試行・自動WSL再入は変更せず、観測不能でも元の失敗を保持する。

Windows回帰は観測/拒否13件、保持12件が成功。実際のシステムPowerShell 5.1でも、
WSLへ入ったり停止したりせず、wslhost.exe 3件・wsl.exe 2件・vmmemWSL 1件・helper 0件を
観測できた。独立したLinuxアーカイブは観測0.21秒・保持0.23秒、共通ローカルCIの
docs19.17秒・workflow policy3.64秒が成功した。root user session警告は修復せず保持した。
製品Goコードとworkflow定義は変更していない。既存の全体検証は元のheadの範囲で再利用する。
これは診断/構成要素の結果であり、新観測での導入済み回収成功や上の失敗の修正完了ではない。

#706 head `38aeae56`で初めて導入済みの観測を実行し、上記の失敗を記録した。
他の必須workflow4件は成功。約6.6秒でWindowsから見える`wslhost.exe`と`wsl.exe`が
0になり、約12秒で`wsl.exe`が2、約17.6秒で`wslhost.exe`も2へ戻った。
新しいWindows側WSLプロセスの証拠だが、対象distributionや起動元は未確定。
後続変更ではWSLへ入らず親の分類を追加し、製品動作は変えていない。
Windows PowerShell 5.1の実クエリにfixtureを渡す回帰と他14件が成功した。
読み取り専用の実プロセス照会も成功し、手元のWSLプロセス4件はPowerShellの子孫だった。
手元の結果でCIの再起動元を特定したとは扱わない。
後続のLinuxアーカイブは観測14件0.50秒（Windows専用クエリ1件はSKIP）、保持12件0.61秒、
共通ローカルCI docs20.78秒・workflow policy1.86秒が成功した。同じroot user session警告は残した。

`c3a2cc86`のWindows初回job104542349177は製品導入前に、固定Microsoft VS Code ZIPの
応答が途中で終了して失敗した。Windows native単体は成功、導入以降はSKIP。
失敗jobを一度再実行すると同じアーカイブの取得・検証、導入、HTTPS、SSH/エディタ、
Linux回収は成功したが、公開回収は上記の失敗となった。約6.9秒で起動/hostプロセスが消え、
約29.1秒で`other` / `wsl/other`の親を持つ起動プロセス2件が再出現した。
SSH・エディタ・端末・容量回収としては分類できていない。初期の分類に`haco-review.exe`が
含まれていないため、Hacocoon自身を原因から除外できない。後続では導入済みの通知/clientと、
既知のWindows側WSL host/relay/serviceの固定分類を追加した。製品動作や期限は変更していない。
分類追加後は実PowerShellクエリを含むWindows回帰15件が0.52秒で成功。
統合Linuxアーカイブは観測14件0.21秒（Windows専用1件SKIP）、docs11.48秒・workflow policy2.16秒が成功した。

### 現行Git接続記録の棚卸し

M5の棚卸し修正は、保存済みの単一レポ・複数レポ接続を既存のリポジトリ参照抽出で表示する。
Windowsは25件成功・Linux専用7件SKIP。Linuxの独立アーカイブでは棚卸し32件0.36秒、
関連付け18件0.28秒、選定7件0.15秒、共通ローカルCI docs13.75秒・workflow policy1.70秒が
成功した。root user session警告は残した。

専用WSLの現行データを読み、リポジトリ記録2件と接続記録1件を抽出できた。
以前は`bindings`が未投影だったが、今回は参照の投影完了・未確認ディレクトリ項目0件。
結果は非公開で保持し、元の記録・サービス・Policyは変更していない。
`authority`と接続の`state_validated`はfalseのまま。接続の現在の有効性、必要データの
選定完了、取得・復元・認証付き開発の成功を示すものではない。

### SSH・エディタ入口の日英案内

通常のSSH/openの環境選択、準備完了、エディタ再試行、接続設定整理を共通辞書へ
移した。プレビューの引数確認をまとめ、Workspace準備前に修正方法を表示する。
受け付ける操作は変えていない。1,669ファイルの隔離LinuxコピーでCLI・辞書のテストが
12.35秒で成功し、実PTYでの日英の選択・中止・クライアント側SSH設定を確認した。
共通ローカルCI `test`（全Goテスト、vet、保守ツール、通知client）が80.54秒、
`docs`が9.92秒で成功。初回の日本語PTY試験はテスト全体の初期化が言語指定を消すため
...
新しいローカルWSLは当初、導入版 `bfa19ecb`、Ubuntu 26.04.1、Incus 7.0.1で、通常ユーザーのdoctorが
...
head `d8ec1374` のIncus run35090329488では、製品3job
（104774859128・104774859406・104774859443）がすべて成功した。集約job104778087319は失敗。
artifact10444681175では`needs_success=true`、製品試験の失敗や必須stepの未確認なしだが、
60秒後も最後のjobのconclusionがnullで、`incus-owned-btrfs`が未確認となった。
後の確定metadataでは成功している。元の観測失敗を保持し、テストは再実行していない。
metadataの確認上限を180秒とし、未確認job名を失敗出力にも表示する。
回帰試験は遅れて反映する成功、期限切れ、確定した失敗の即時返却、APIエラーを区別し、
`needs`だけで成功扱いにしない。

同じ候補を固定GoReleaser 2.17.1と通常のWindowsパッケージ・インストーラで新WSL
`Hacocoon`へ導入した。client/controllerはともに`d8ec1374`、doctor全6項目成功。
公開CLIでmainを独立Workspaceへcloneし、一時Envから確認用ファイルを作成、削除後に再利用した。
snapshot保存・独立復元・復元先Envだけの削除・新しい一時Envからの利用でも、ファイルSHA-256、
guest所有者`0:0`、Git管理ディレクトリとOCI接続が保持された。小さな保持データ確認であり、
実OCIアプリ・認証付きGit・巨大レポの受入ではない。最初の確認では標準BaseにGit実行ファイルが
なく`git`が失敗したが、最後のhash出力が成功してシェル終了値は0だった。後の確認はfail-fastとし、
Git実行の成功は主張しない。

起動直後に明示的なWSL設定コマンドを一度実行すると管理socket不在となり、設定は変更されなかった。
通常の管理端末を開き、doctor成功と最新設定の確認後に適用を確認した。通常の要承認ルール3件は
`packer-tools`とUbuntu公式HTTP・HashiCorp公式HTTPSの宛先に限定し、2026-09-16T13:42:55Zに失効する。
default denyを維持。その後、Ubuntuパッケージを通常proxy経由で取得できず、
`dependencies`段階でapt終了値100の失敗となった。本人のGUI回答は未確認。
Packer取得・HCL実行・公開・再利用には到達していない。通常CLIで一時Env `packer-tools`が
存在せず、Base `roadmap-tools`も未公開であることを確認した。一時ルール3件はrevisionを
確認して通常設定操作で削除し、元のdefault deny・ルール0件に戻した。暗黙の許可や自動再試行は行っていない。

`d8ec1374` のWindows run35090329311/job104774858507は、導入・HTTPS・SSH/エディタ・
Linux回収が成功したが、公開容量回収は`compact_attached`、359回、圧縮未開始、復帰成功で失敗した。
通知受入はSKIP。6.7秒に起動/host件数が0となり、23.1秒には観測中の起動プロセスなしで
hostが2件現れ、39.4秒で再び0になった。94.2秒の起動は回収workerの復帰処理に由来する。
新しい通知起動は観測されていないが、観測間に短時間だけ起動した可能性は除外できない。
原因は未解決。CIでWSL hostの親分類を同じ上限・固定値で別に観測する。
製品の停止・90秒の切断確認・拒否条件は変えていない。実PowerShell 5.1への投影を含む
Windows観測回帰16件が成功。先行`7e5971b5`の成功でこの失敗を解決扱いにしない。

手元の`d8ec1374`で通常exportした599,424,512バイトをWindowsへコピーし、SHA-256
`0e3fdd21e6eb257b53123f837a1e32cd60fc62d74d21dc311c5f5d62f922c9da`の一致を確認した。
Windows投影パスからの通常importは独立した管理データを作成。import先Envだけを削除し、
新しい一時Envからも同じ確認用ファイルのhash・guest所有者・Git管理ディレクトリ・OCI接続が
残ることを確認した。元のsource・snapshot・bundleは保持。同一PCの小さな転送1件の確認であり、
別PCや認証付き開発の成功にはしていない。

同じローカル導入候補で、新WSL `Hacocoon`の通常`haco reclaim`が成功した。
操作`1c487b24-df95-4cc6-85fe-cb0f97d490ae`でWindows割当量は9,603,907,584から
7,861,174,272バイトとなり、1,742,733,312バイトを回収。open 254回、仮想容量は不変、復帰成功。
通常の結果表示でLinux両段階とWindowsの完了を確認し、復帰後のdoctor全6項目も成功した。
独立import先Workspaceの一時Envでは、確認用ファイルのhash・guest所有者`0:0`・Git管理
ディレクトリ・OCI接続が保持され、片付けも成功した。WSL全体終了や強制切断は使っていない。
以前のローカル登録確認失敗とCIの接続中ディスク失敗は、この成功で解決扱いにしない。

回収後の最初の確認では、停止中`roadmap-save`が使用権を持つ元Workspaceを選んだため、
`storage has active sessions`で作成が正しく拒否された。CLIはこの理由を出さず、
片付け未確認の案内だけを表示した。試行先Envの不在を確認し、元Envと使用権を保持したまま
独立import先で確認を完了した。後続修正では通常コマンドと固定の失敗分類を共有し、日英で
`busy`の理由と次の操作を案内する。JSON・終了値・片付け結果・生の実行基盤エラーの非表示は維持する。
理由・使用権案内の回帰試験は修正前失敗・修正後の日英成功を確認し、共通ローカルの
`test`・`docs`・`workflow-policy`と対象CLIのraceも成功した。開発CLIを導入済みcontrollerへ
接続した確認でも、同じ拒否で日本語の理由・使用権案内が表示され、保持中Envは元の1件のままだった。
これはCLIと導入済みcontrollerの確認であり、新しいインストーラや配布版の受入ではない。

`423fa602`ではtest35093944048・quality35093943917・Ubuntu35093944270・
Incus35093944417（集約判定を含む）が成功した。Windows run35093944130/job104786573377は
導入・HTTPS・並列cold SSH・実VS Code編集・export/import・Env削除後の作業再利用が成功。
転送はWindows listenerの所有元と1 MiB×8接続の交換まで成功したが、Ctrl+C後に一般的な
Windows接続失敗と終了値1を返し、端末試験が終了値0を待って期限切れとなった。
回収・通知はSKIPで、新しいhost親分類の結果はない。main未統合、同headの再実行なし。

手元の導入済み`d8ec1374`は同じ通常Windows端末操作で、listener所有元・データ交換・
半切断・Ctrl+C後の接続終了まで成功した。最初のローカル試行はpywinpty不足で端末を
起動できず、CIと同じ固定3.0.2を試験用ディレクトリへ準備した後に、製品の補修・Policy変更なしで実行した。
CIの中断失敗は未解決。子プロセスの失敗段階・固定理由・中断状態・経過時間を記録するようにし、
原因を推測して終了値を成功へ変えたり、10秒の強制停止期限を変えたりしない。

<a id="incus-key-download"></a>

## Incus署名鍵の接続失敗

`1054688e`ではtest35097958384・quality35097958387・Ubuntu35097958415・
Windows35097958377が成功した。Windows job104799915515では通常転送のnative所有元・
1 MiB×8接続・半切断・Ctrl+C後の終了を確認。通常の容量回収は2,790,260,736バイトを回収して
復帰し、導入済み通知経路の段階も成功した。本人による承認回答ではなく、以前の断続的な
転送終了失敗・接続中ディスク失敗の原因が解決した証拠でもない。

Incus35097958382は失敗。standalone job104800045423は署名鍵の取得時に
`pkgs.zabbly.com:443`へ接続できず（curl終了値7、207 ms）、製品試験前に停止した。
owned-Btrfs104800045764・Core104800045879は成功し、集約判定は正しく失敗した。
同一headの再実行やmainへの統合は行っていない。

製品・CI共通の導入処理で、一時的な失敗時の署名鍵取得だけを上限付きで再試行する。
HTTPSと固定primary keyの照合は維持。隔離TLSサーバーと実curlによるローカル試験で、
503からの復旧・再試行上限・恒久的な404・復旧後の不正鍵によるHost変更前の拒否を確認した。
...
先行する#708のhead `a67a982acaaacb8f446db338d8b40808e096b467` は必須5ワークフローが
成功し、main `63bc41d14336b8c50895b4f689b12e2a49583a31` に統合された。
Windows run35110190568/job104841576457では割り当て容量2,805,989,376バイトを回収し、
同じWSLへ復帰した。保持データ、通常転送のCtrl+C終了、通知経路も成功。
実イベント観測で停止から復帰まで新しいWSL起動は見つからなかった。
この成功は、上記の過去候補での失敗原因を説明するものではない。

そのmainに追加した実Windowsの回帰テストで、接続準備失敗と `peer.Close` の後も
孫プロセスが書き込める不具合を再現した（2.21秒、失敗）。専用Jobでの修正は
`aec8d4bca6ed9f07b2ad74c76f99ac6895082442`。その後、通知パッケージのnative試験は
3.86秒で成功した。遅延した孫、先に終了した起動プロセス、別接続への非干渉、
キャンセル、起動排他を含む。対話通知セッションを有効にしていないため通知表示試験は
SKIPであり、人のクリックは未確認。Windows amd64のテストビルド・arm64ビルド、
Linuxの共通通知・転送パッケージも成功した。一時的な実機テストで、導入済み
`d8ec1374` のHacocoonに `SessionPlan("Hacocoon")` で読み取り接続し、終了確認まで
0.06秒で成功した。導入済みバイナリの差し替えや承認回答は行っていない。
今回の修正を含む配布相当確認は未完了。

別途、導入済み `d8ec1374` で通常Windows転送を3回実行し、native所有確認、
8本×1 MiB交換、片側終了、Ctrl+C後の解放が成功した。4回目は交換前に
`Get-NetTCPConnection` の観測が15秒を超えて停止した。これは未解決の観測失敗であり、
Ctrl+Cでの失敗ではない。通常Host入口を通したパイプだけの限定試験では、
実WSL→WindowsのCtrl+Cを25回確認できた。最初の限定試験は外側WSLから直接実行して
exec-format errorとなり、キャンセルの確認には至らなかった。これらの成功で、
過去の `423fa602` の転送非ゼロ終了の原因が確定したとはしない。

手元では別distributionが稼働中であり、触れずに残したため、新たな公開容量回収は
実行していない。実Windows起動イベント購読もOSのアクセス制御により利用できなかった。
以前の `a0303de9` の接続中ディスク失敗は未解決のまま保持する。今回の所有不具合は
再現して修正できたが、すべての間欠的失敗と同一原因である証拠ではない。
[ADR 0110](../adr/0110-private-windows-process-ownership.ja.md)を参照。

## M2のローカルGit接続診断

実装commit `9635e2b0`のprovider確認処理を、`Hacocoon` WSL内の読み取り専用probeから
既存の停止中`roadmap-save`へ適用しました。コントローラーの環境情報と記録済みWorkspaceを
使い、所有する接続先を認識し、別のbroker socketと別のWorkspace所有者は拒否しました。
資源・接続・レポジトリ・Policyは変更していません。通常ユーザーの直接Incus確認は権限不足で
実行できず、providerの確認はIncus管理者で行いました。通常ユーザーのデスクトップ経路や
起動中Envの実修復の受入ではありません。認証付きGit・GUI回答・実機の修復操作は未確認です。

M2の初回パッケージ試験では、大容量Git試験の10秒以内の承認要求待ち失敗（試験全体約88秒）と、
既存PTY bootstrapの5秒待ちの時間切れが残りました。原因を断定せず、集中試験の成功で消しません。
同じ実行で発見した重複`--json`の拒否仕様の回帰は修正し、その後のGit診断、API往復、providerの
接続確認、オフライン・不一致の回帰と通常コマンドE2Eは成功しました。以前のM1の管理ツール試験の
時間切れも別件として残し、全体試験は繰り返していません。

## M1/M2の統合条件

M1 #726のhead `5b447f9c`は4つのworkflowに成功しましたが、Windows run36152506002
（job108128998385）の公開容量回収が失敗しました。Linux側は完了し、359回のnative open後も
`compact_attached`で、圧縮は未実施、再開は成功、通知の確認はSKIPです。待機中にWSLの
起動が観測されましたが、起動元不明もあり、原因プロセスの特定や別distributionの停止を
正当化する証拠ではありません。原因は未解決で、以前の成功で失敗を消しません。

M2 #727のhead `e5dfeaa8`も4つのworkflowに成功しましたが、Windows run36156927382
（job108143623213）のdoctor判定が失敗しました。SSH試験は外部パスのWorkspaceを作るため、
Git診断の`not_applicable`は正常です。既存判定が`ok`以外をすべて拒否していました。
修正後は`git_broker`だけこの状態を認め、CLIの正常終了を要求し、failed・skipped・unknownは
引き続き拒否します。native PowerShellの30判定と構文確認は成功しました。広い診断スクリプトは
既存の子プロセス3秒制限の証拠確認で失敗し、別件として保持します。実機E2Eの成功とはしません。
両PRとも必要なexact-head workflowが揃うまで未マージです。

その後、ファイルシステムsandbox外でnative診断スクリプトを実行し、子プロセスの時間切れと
非ゼロ終了の回帰は成功しました。先のローカル失敗は残し、実機E2Eの成功とは扱いません。

## エディター試験の子プロセス回収

M1 #726のhead `5b447f9c`はWindows run36152506002/job108128998385で、Linux容量回収の
成功後に失敗しました。359回のnative open後も`compact_attached`で、圧縮は未実施、
同じ対象の再開は成功、通知はSKIPです。起動イベントには起動元不明もあり、再起動の所有者は
特定できていません。別distributionの停止を正当化する証拠ではありません。

調査で、`test_vscode_environment.ps1`が専用エディター本体だけを終了し、SSHなどの
子プロセスを回収していない不具合を発見しました。native Windowsの親子プロセス試験で、
旧処理では子が残ることを再現しました。修正後は選択したプロセスの子も終了して親の終了を待ち、
同じ試験で子の終了・無関係なプロセスの存続・二度目のcleanup成功を確認しました。
試験用エディターの完全一致する実行ファイルだけが対象で、利用者のエディターやWSLを停止しません。
...
統合PR #727のhead `4459418c`は4 workflowに成功しましたが、Windows run36164722897は359回open後に`compact_attached`で失敗し、圧縮未実施・再開失敗・通知SKIPでした。保存記録に再開のエラー詳細がなかったため、このcheckoutでは最初の失敗を維持して固定分類と数値コードを別途保存します。過去の失敗は未解決のままで、新しい実機結果は確認待ちです。

ローカルのCLI状態表示・日英表示の回帰に成功しました。Windows実プロセスの終了コード`0x8000FFFF`、再開失敗の分類、元の失敗を維持した保存、既存の操作記録・reviewのnative回帰も成功しました。CI出力の21テストに成功しました。この変更で導入済みWSLの停止・圧縮試験は実行していません。最初の検証準備はshell引用と再起動後の一時ソース不在で失敗し、準備を修正した後に集中試験が成功しました。
