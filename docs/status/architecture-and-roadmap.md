...
reclamation/operability and G export/migration remain planned. Internal F1 now connects controller Linux discard to the enrolled Windows worker, preserving per-stage evidence and refusing shutdown after Linux failure. Public dispatch/status is implemented; combined native acceptance and interrupted-pending review remain partial; see the [reclamation contract](../design/storage-reclamation.md). These remaining stages preserve data and permissions while using
