...
| M3: environments beyond the usual network | **partial**. Reuse #576 TCP/UDP plus its expiry/revocation/generation tests. Temporary-run stdin/TTY is implemented on the development candidate, with Incus 7.0.1 pipe/PTY and ordinary Windows ConPTY gates passing at `9767fd93`. Client UDS/TCP forwarding is implemented at `640c66ff`; its installed acceptance and Windows-native listener path remain. The dedicated Windows-service timeout and VPN/DNS modes remain. No firewall exception is introduced to obtain a pass. |
| M4: large repositories | **partial**. Preserve existing Base builder/CoW/OCI results. [Actual guest-only Packer HCL2/external shell](../design/packer-base-builds.md) (#566) is implemented in this candidate, with installed acceptance open. [Cache generation selection and detached provider copies](../design/cache-generations.md) are a partial foundation; Host settings, normal-Env path collection/placement, disposable attachments/history/clearing and real cache acceptance (#570), cleanup and actual large-repo measurement (#241) remain. Small #579 fixtures are not giant-repository acceptance. |
...
Incus/Windows acceptance passed at `9767fd93`; M3 remains partial. Continue client forwarding and VPN/DNS gaps in environments beyond the usual network independently. [ADR 0069](../adr/0069-bounded-process-streams.md) owns
...
- M3: automatic WSL-entry delegation is implemented in `codex/windows-tunnel-entry`;
  installed Windows TCP interruption/cleanup passes at #642; Host/backend/disabled DNS modes remain and checks in environments outside
