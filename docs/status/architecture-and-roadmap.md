...
| M4: large repositories | **partial**. Preserve existing Base builder/CoW/OCI results. [Actual guest-only Packer HCL2/external shell](../design/packer-base-builds.md) (#566) is implemented in this candidate, with installed acceptance open. Remaining: Host-selected normal-Env cache generation/COW reuse (#570), cleanup and actual large-repo measurement (#241) remain. Small #579 fixtures are not giant-repository acceptance. |
