...
v0.67内で、制限された失敗段階と日英の次の操作案内を通常のWindows容量回収に追加する。
M5完了やリリース公開ではない。既存のディスク使用中の失敗と、導入環境での起動前の失敗は
それぞれ未解決の実機確認として保持する。

## 復元ツリーの照合

v0.67のM5候補で、既存の保守用退避手順に持ち運べるファイル一覧の比較を追加します。
復元確認の一工程であり、移行全体の受入やリリース公開ではありません。

## 容量回収の言語範囲

v0.67内で結果と確認の日英表示を共通化し、日常利用の表示不足を補います。
通信形式、承認、ライフサイクルは変更せず、リリースは公開しません。

## 環境名からの復元

v0.68では保存開始日時から最新の完了済みsnapshotを選べるようにした。
復元は既存の共通処理で独立したデータを作成する。ローカル回帰と導入済みIncusでの
一連操作が成功した到達点であり、M5全体の完了やリリース公開ではない。

カタログ単位のライフサイクルロックへの修正はv0.68の利用上の不具合修正であり、新しい到達点やリリースではない。本人ログインやGUI操作は明示的にリリース後の確認とし、実装済みの変更をmainへ入れる条件にしない。

複数headのfetchを順に取り込む変更もv0.68の利用上の修正とする。個々のpackとrefごとの認可を維持して合計サイズでの拒否を取り除くもので、巨大レポの性能確認を完了したことにはしない。

セットアップ結果の日英案内はv0.68の日常利用の修正であり、段階番号の更新やリリースは行わない。
本人認証、通知・エディタ内の人による回答はリリース後の確認として残し、
CI成功済みの実装をmainへ反映する条件にはしない。

main e4d99700（#699）へ保持データ・Git・ライフサイクルの追補を統合した。同じheadの5系統CIが成功しており、v0.68内のmain反映である。公開や本人操作の確認完了とは区別する。

通信と設定の結果案内はv0.68内の日常利用の修正である。日英表示によって通信権限を変えず、
段階番号の追加やリリース公開も行わない。

Gitの順次転送候補はv0.68内の利用改善として扱う。32 MiBのpack制限撤去は
巨大レポ性能の確認やリリース公開を意味しない。
...
M2のGit接続診断・明示的な修復はv0.68の使いやすさの修正です。既存の所有権確認を再利用し、チェックポイントの更新・pushの許可・リリースは行いません。

今回のM1のみの追加作業は、v0.68の日常利用の修正です。補助クライアントの日英案内とヘルプのみでの正常終了に対応します。チェックポイントの更新やリリースは行いません。人によるデスクトップの確認とCLI全体の翻訳は別途残ります。
