...
後続のcold-doctor待機修正は `2d79778` でfresh Windows gateが成功した。現在の実機installでもcontroller round tripは成功したが、最初のcold疎通検査は失敗し、後の通常入口で成功した。原因は未確定。その後の診断cause/action追加にはrepository検証があり、独自のpackage受入はpending。Windows再起動、広い層別診断、install済みEnvironment proxy制御はM1の残件である。証拠と制約の正本は[実装status](../IMPLEMENTATION_STATUS.ja.md)。
