...
M0–M1 remains **partial**. The earlier Incus startup SIGKILL source is unresolved despite later successful runs. Broader layer diagnostics, reboot continuation/Windows reboot acceptance, firewall ordering and installed Environment allowed-proxy/denied-direct traffic remain. [Implementation status](../IMPLEMENTATION_STATUS.md) owns the commit-bound evidence, package identity and acceptance limits.
