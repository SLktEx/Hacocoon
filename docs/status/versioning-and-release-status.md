...
The subsequent cold-doctor readiness wait passed the fresh Windows gate on `2d79778`. On the current local installation its controller round trip passed, while the first cold connectivity check failed; a later ordinary-entry check passed. The failure cause remains unconfirmed. The later diagnostic cause/action projection has repository coverage and awaits its own packaged acceptance. Windows reboot, broader layer diagnostics and installed Environment proxy enforcement remain separate M1 work. Exact evidence and limits are owned by [implementation status](../IMPLEMENTATION_STATUS.md).
