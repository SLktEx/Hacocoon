...
The subsequent cold-doctor readiness wait passed the fresh Windows gate on `2d79778`. On the current local installation its controller round trip passed, while the first cold connectivity check failed; a later ordinary-entry check passed. The failure cause remains unconfirmed. The later cause/action projection passed Windows, Ubuntu and all four Incus hosted jobs on `663e2cd`; its local BAT/ordinary-entry/retention checks passed. Local direct cold execution returned exit 9 once and timed out after a confirmed stop while Incus was still activating. The two-minute controller wait passed the fresh Windows gate and local exact BAT application/rerun, ordinary entry, cold doctor and retention checks on `32592fb`; the separate exit-9 cause remains unconfirmed. The subsequent Standard proxy lifecycle reuses the controller, couples shutdown and refuses ambient daemon approval. Windows/Ubuntu/all four Incus gates passed on `abd4125`; local package application/rerun, ordinary entry, retention, same-PID listener and unmanaged-source refusal passed. A local cold DNS failure preceded guest resolved/DHCP startup. The read-only guest prerequisite wait passed the Windows gate and local cold doctor on `42e2fb3`, while a later ordinary-entry attempt exposed an Incus SIGKILL/start-post stall. Standard automatic recovery and subsequent retained-state checks passed; the signal source remains unconfirmed. The next storage diagnostic slice separates configured policy, verified live application and pending application, without a second mount lifecycle; its package acceptance is pending. Windows reboot, broader layer diagnostics and installed Environment proxy enforcement remain separate M1 work. Exact evidence and limits are owned by [implementation status](../IMPLEMENTATION_STATUS.md).
