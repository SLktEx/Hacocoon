...
現在の製品hacoはコントローラー経由のsetup/doctor、WSL通常入口、repo・Workspace・Environment・Git・OCI Storeの操作を提供する。旧 CLI の廃止判断は [ADR 0107](../adr/0107-responsibility-layout-and-cli-retirement.ja.md)を参照する。
...
製品 `haco` はコントローラーを呼び、guest-local 構成を作りません。旧 CLI は新しい配布物と trusted-host 配備から削除しました。[ADR 0107](../adr/0107-responsibility-layout-and-cli-retirement.ja.md)を参照してください。
...
実装済みの候補: 通常のWindows/WSLの対話起動時にWindowsの表示言語を一度読み取り、
`en`または`ja`だけを信頼されたHostセッションへ渡します。明示的な`HACO_UI_LANGUAGE`を
最優先し、`LC_ALL`または`LC_MESSAGES`が指定されている場合もWindowsの自動判定より
優先します。それ以外の起動や取得失敗時にはPOSIXの言語設定を使います。入口の案内と
HostのCLIは同じ言語です。Host権限を使う場所であることと、普段の開発にはEnvを使う
案内を維持します。対話端末のstderrは`NO_COLOR`が空なら黄色にし、転送時は色を付けません。

インストーラは新規・既存ディストリビューションのOS/WSL言語設定を保持します。
表示言語の選択でツール全体のlocaleを変えず、任意の環境変数や通常Envのshellへは
引き継ぎません。[言語設定](../reference/cli-language.ja.md)と
[ADR 0079](../adr/0079-host-presentation-language.md)を参照してください。
日本語Windowsの配布パッケージによる起動確認は、読み取り専用の言語取得テストと区別します。
