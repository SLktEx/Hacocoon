...
現在のデータの保守では、管理対象データ・Host設定・手動ファイル・外部データに名前を付け、
`tools/evacuation_selection.py` で既存のツリーマニフェスト照合をまとめられる。
読み取り専用の開発用ヘルパーであり、import・保存・ライフサイクル処理は追加しない。
再作成・除外の判断を明示的に残す。選択した範囲の一致だけでは棚卸しの網羅性、独立した保存、
アプリの利用確認、元データの削除権限を証明しない。[対象選択と照合](../guides/data-evacuation.ja.md)を参照。
...
exportはversion 2を使い、リポジトリ名とGitHub接続情報を保存します。
取得開始前に、保護されたWorkspace情報の読み取り処理が設定済みであることを要求します。
設定漏れによってリポジトリ対応表を持たない形式へ切り替えません。version 1は引き続き読め、
