...
内部に Windows 圧縮処理を追加しました。保持中の file から volume GUID のパスを取得し、
親ディスクを辿らず VHDX を開き、動的かつ未接続であることを要求します。native の完了と
実割当量を分け、圧縮後に仮想容量・識別子を照合します。キャンセルを同期処理の取消完了とは
扱いません。この経路は **未公開・受け入れ未完了** で、公開操作には接続しません。

キャンセル・通常ファイル拒否の回帰は成功しました。専用の実 VHDX 試行は4.27秒で
`OpenVirtualDisk` の共有違反により失敗しました。元の file・親 handle を保持したままの
結果で、`CompactVirtualDisk` は未実行です。成功させるために固定を解除していません。
native virtual-disk handle への安全な実体照合の引き渡しは未検証・残課題です。

失敗後、同じ登録 ID の専用 WSL を再開しました。新規確認ファイルの hash と9件の停止中
instance の名前・状態は事前記録と一致しました。これは再開とその観測の検証であり、全保存
データの内容保持や圧縮成功の主張ではありません。確認ファイルは次の検証用に
`/var/lib/haco-reclaim-compact.7NTvx7` に残しています。試行前の Windows ファイル長・
実割当量は8,373,927,936 byte で、回収量の成功報告はありません。
