...
現在の操作記録がない場合、`haco reclaim --status` は「保存された結果がない」と明示して成功します。読み取り時の観測であり、過去に回収が実行されていないことや完了の証明ではありません。操作やレジストリキーは作成せず、`--review` も確認対象なしとして終了します。不正な記録、アクセス失敗、明示した操作IDの不存在は引き続きエラーです。保留・失敗の記録を不存在へ置き換えません。
