...
Environment
  ...
  -> Policy・承認・監査
  -> Host の DNS 解決と許可されたユニキャストアドレスの固定
  -> 許可した上流
```
...
- 認可済みホスト名は、RFC1918 の IPv4、IPv6 ULA、共有・ベンチマーク用ネットワークを含む公開・私設のグローバルユニキャストアドレスへ接続できます。アドレス分類は通信上の検査であり、別の Policy 判断ではありません。分類には Go の `netip.Addr.IsGlobalUnicast` を使いますが、インターネット上の到達性を保証するものではありません。
- ループバック、リンクローカル、マルチキャスト、未指定、不正形式・ゾーン付きの応答と、予約済み IPv4 `0.0.0.0/8`／`240.0.0.0/4` は拒否します。Standard プロキシは Physical Host のコントローラーから接続するため、`127.0.0.0/8` と `::1` はその Host のループバックを指します。リンクローカルはインターフェースの指定が必要で、Host 内の基盤サービスに届く可能性があります。予約済み・非ユニキャストは通常のユニキャスト接続先ではありません。IPv4 射影 IPv6 は IPv4 に戻して検査します。
- 公開・私設の混在は許可します。拒否対象が一つでも含まれる場合は、応答順にかかわらず接続前に集合全体を拒否します。重複を除き、接続失敗時は固定した次のアドレスを試します。DNS は再解決しません。
- 実行基盤と SSH の設定は `NO_PROXY=localhost,127.0.0.1,::1` と小文字の `no_proxy` を維持します。localhost 通信は Environment 内に留まり、プロキシ経由のホスト名に Physical Host のループバックを許可しません。[ADR 0110](../adr/0110-private-egress-and-loopback-isolation.ja.md)を参照してください。
...
上流の失敗は HTTP／CONNECT の境界で一度だけ ERROR に記録し、認可したホスト名、
ポート、Environment と固定の理由を残します。DNS 解決失敗、空の応答、ループバック拒否、
その他のアドレス拒否、接続失敗を区別し、キャンセル・期限超過を優先します。HTTP 応答は
一般的な表現を維持します。DNS・接続処理の生のエラー、IP アドレス、URL、ヘッダー、本文は
ログに含めません。[ログの契約](../reference/logging.ja.md#プロキシ上流の診断)を参照してください。
...
私設・公開ユニキャストの混在許可、危険な応答を含む集合の全体拒否、私設宛ての
HTTP／CONNECT、実行基盤と SSH の localhost 迂回設定もコンポーネント試験で検査します。
導入済み環境での私設ネットワークや、ゲスト内の `curl localhost` の実機受け入れを
証明するものではありません。
