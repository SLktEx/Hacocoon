...
starts a supported `ssh.service`/`sshd.service`, validates sshd and installs only
the managed client public key. Connection transport remains the portless
controller stream described above.
