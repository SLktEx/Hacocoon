...
旧`haco base build base.json`は、`name`・任意の`from`・長さを制限した`run`を使う移行用経路として
残します。HCLをこのJSONへ変換する必要はありません。非公開の制御要求では構築方式を一つだけ指定でき、
どちらも同じライフサイクル・lease・Incusのカタログを使います。
[ADR 0075](../adr/0075-guest-packer-provisioning.ja.md)を参照してください。
