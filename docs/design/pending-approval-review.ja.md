...
読み取り専用の二重起動は、既存helperの「要求は終了済み」を独立した拒否結果として維持します。
controller／表示失敗は利用不能のままです。どちらも表示成功の応答や回答送信にはなりません。
native失敗の診断には固定の段階名と型付きのnative状態数値だけを使います。

通知描画のキャンセルと期限切れは別の失敗理由として保持します。子が停止前に成功markerを出していても、
期限内の完了とは扱いません。既存の8秒の処理上限と所有する子の回収は維持します。
失敗logには固定reason／native statusに加え、子の数値終了コードと経過ミリ秒を記録します。
未知の子出力は非公開のままで、承認結果にしません。この診断改善だけで、過去の導入済み`stage=clear`失敗の
原因判明や新規回答の受入完了とは扱いません。
