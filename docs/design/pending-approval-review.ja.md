...
native PowerShellプロセスの初回起動と最初の所有通知クリアには最大40秒を設けます。個々の回答画面・読み取り・終了時cleanupは呼び出し側の短い10/8秒の上限を維持します。中断は成功表示より優先し、起動した子だけを終了します。起動が遅くてもクリアを省略せず、承認を再送したり、失敗を成功へ読み替えたりしません。
