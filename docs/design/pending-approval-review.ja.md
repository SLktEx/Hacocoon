...
native PowerShellプロセスの初回起動と最初の所有通知クリアには最大30秒を設けます。個々の回答画面・読み取り・終了時cleanupは呼び出し側の短い10/8秒の上限を維持します。中断は成功表示より優先し、起動した子だけを終了します。起動が遅くてもクリアを省略せず、承認を再送したり、失敗を成功へ読み替えたりしません。


COMの表示受付前に、WSLとの読み取り専用応答で準備完了を確認します。
所有履歴clear・準備確認・最初のreviewは起動待ち50秒を共有します。
重複した起動は、表示の準備完了か、前の処理のcleanup終了後に所有権を得るまで
最大60秒待ちます。通知元は引き継ぎと起動に最大120秒を設けますが、通常の
10/8秒の読み取り・回答期限は変更しません。ユーザー別のmutexが使用中でも、
COMで表示できるとは判断しません。初期化完了後に名前付きeventで準備完了を
示します。このeventに回答や認証情報は含みません。
終了時は準備完了の表示を取り消してからCOM登録を解除し、private peerと所有履歴の
