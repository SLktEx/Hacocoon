...
Windows通知の確認は、容量回収とdistributionの起動排他を共有する。初期接続・
回答用接続は期限付きの読み取り確認を終えてから解放し、失敗時は先に子を回収する。
競合時は新しい接続を拒否し、WSL起動・承認回答・再送を行わない。既存リクエストは
通常の有効期限・識別確認に従う。容量回収が終わってから確認を開き直す。
競合の根拠と調整の限界は[ADR 0108](../adr/0108-background-wsl-start-coordination.ja.md)を参照。
