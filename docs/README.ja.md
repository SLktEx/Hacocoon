...
このファイルは、2026-08-29 の architecture rebaseline と、その後の **v0.8 までの実装進行**を踏まえて、Hacocoon の資料をどの順番で読めばよいかを説明します。

> [!NOTE]
> 日本語資料は読みやすさのための補助資料です。設計上の最終的な正本は英語版の authoritative document です。

Hacocoon はまだ **pre-1.0** です。現在の architecture や実装済み roadmap が書かれていても、API / CLI / state format / provider / client adapter / configuration の互換性保証ではありません。
...
`README.md`、`CODEX_START_HERE.md`、`Hacocoon_v0.1-v0.7_MASTER.md` は入口です。最後の master filename は historical name であり、v0.8 の正本を上書きしません。

## 現在の読み方

現在の実装進行は v0.8 です。

- v0.1〜v0.8 spec は **versioned design contract**。
...
Breaking Change の対象には CLI、helper binary、state、Core / adapter API、Capability / Policy、Provider、Client Adapter configuration、experimental backend、document structure が含まれます。
