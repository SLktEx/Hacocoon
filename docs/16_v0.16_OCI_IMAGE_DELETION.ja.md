...
Status: **first deletion sliceは`main`に実装済み。immutable Seed rebuild/GCはv0.19。**

v0.16はtrusted Host Seed cacheと、明示指定時のmanaged Environmentに対するOCI image deletion semanticsを提供します。
...
5. immutable Seed replacementとold-Seed GCはv0.19へ委譲
