...
v0.19 owns the physical OCI Seed pipeline: trusted Host-side image acquisition, offline Seed construction, immutable publication, revision pinning, and storage-driver COW benefits.
...
Seed observation/deletion remains under `haco plugin oci ...`. The physical v0.19 builder/publisher must also remain outside Core behind the OCI/provider adapter boundary; it must not turn containerd, nerdctl, OCI manifests, Incus images, or Btrfs subvolumes into Core vocabulary.
...
- v0.18 may optionally provide a Registry/proxy but is not a prerequisite.
- v0.19 performs the actual immutable Seed build/publish/COW lifecycle.
