...
func TestOfficialBaseDoesNotFallBackToUpstreamWhenBuilderRevisionIsMissing(t *testing.T) {
	runner := &fakeRunner{run: func(_ context.Context, _ int, _ string, args []string) (host.Result, error) {
		joined := strings.Join(args, " ")
		if strings.Contains(joined, "/1.0/images/aliases?") {
	...
	_, err = provider.resolveBase(context.Background(), defaultBaseName)
	if !errors.Is(err, core.ErrNotFound) {
		t.Fatalf("error = %v, want ErrNotFound", err)
	}
	for _, call := range runner.calls {
		joined := strings.Join(call.args, " ")
		if strings.Contains(joined, "images:ubuntu/26.04") {
			t.Fatalf("ordinary official Base resolution fell back to upstream parent: %#v", call)
		}
	}
}
