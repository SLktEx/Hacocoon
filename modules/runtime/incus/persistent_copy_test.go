...
func TestPersistentCopyUsesOfflineBtrfsAndNewOwnership(t *testing.T) {
	for _, kind := range []string{OCIStoreKind, CacheResourceKind} {
		t.Run(kind, func(t *testing.T) {
			source := core.PersistentResource{ID: "oci:source", Kind: kind, Owner: strings.Repeat("a", 32), NativeRef: "pool/haco-persistent-" + strings.Repeat("a", 32)}
			target := core.PersistentResource{ID: "oci:target", Kind: kind, Owner: strings.Repeat("b", 32), NativeRef: "pool/haco-persistent-" + strings.Repeat("b", 32), CopySource: source.Ref()}
