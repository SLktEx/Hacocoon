...
func (r *Runtime) DiagnoseHost(ctx context.Context, storage BtrfsLoopPoolSpec) (diagnostics.Report, error) {
	...
	check(1, "Configured Btrfs pool and mount policy match",
	...
	hostOK := check(2, "Owned trusted host is running with its explicit root, NIC and controller endpoint",
	...
	networkOK := check(3, "Owned trusted bridge has the required DNS, DHCP, NAT and firewall configuration",
	...
	if hostOK && networkOK {
		...
		if !ok {
			...
			case 21:
				report.Checks[4].Summary = "Trusted-host IPv4 DNS lookup for github.com failed"
				report.Checks[4].Action = "Check Physical Host DNS and the owned bridge DNS service, then run haco doctor again"
			case 22:
				report.Checks[4].Summary = "Trusted-host default IPv4 route is unavailable"
				report.Checks[4].Action = "Check the owned bridge DHCP service and trusted-host NIC, then run haco doctor again"
			case 23:
				report.Checks[4].Summary = "Trusted-host HTTPS to github.com failed after DNS and route checks passed"
				report.Checks[4].Action = "Check Physical Host HTTPS reachability and forwarding for the owned bridge; keep Environment direct traffic denied"
			}
		}
	}
	return report, nil
