...
// Environment. Official Base names resolve only to the private revision produced
// by the trusted Base Builder. They never fall back to the upstream parent; the
// builder itself explicitly uses resolveParentBase through ParentBaseOnly.
func (p *BaseProvider) resolveBase(ctx context.Context, requested core.BaseName) (resolvedBase, error) {
	...
	if buildName, ok := basebuild.OfficialBuildName(name); ok {
		...
		return resolvedBase{}, fmt.Errorf("official Base %q has not been built: %w", name, core.ErrNotFound)
	}
