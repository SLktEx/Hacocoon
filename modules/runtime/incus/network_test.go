...
func sandboxProfileResult() host.Result {
	proxy := "http://" + sandboxRoutedHostIPv4 + ":18080"
...
func sandboxNetworkResult(args []string) (host.Result, bool) {
	// Routed host address inspection.
	if len(args) >= 4 && args[0] == "-o" && args[1] == "-4" && args[2] == "address" && args[3] == "show" {
		return host.Result{Stdout: "1: lo    inet " + sandboxRoutedHostIPv4 + "/32 scope host lo\n"}, true
	}
	...
	// All nft mutations are privileged; tests model a pre-existing exact
	// Hacocoon table so they exercise verification rather than host mutation.
	if len(args) >= 7 && args[0] == "-n" && args[1] == "--" && args[2] == "nft" && args[3] == "list" && args[4] == "table" && args[5] == sandboxRoutedFirewallFamily && args[6] == sandboxRoutedFirewallTable {
	...
	if len(args) >= 6 && args[0] == "config" && args[1] == "device" && args[2] == "get" && args[4] == "eth0" && args[5] == "ipv4.address" {
