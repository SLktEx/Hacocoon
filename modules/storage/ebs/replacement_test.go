...
func (f *fakeAPI) DescribeVolume(context.Context, string) (Volume, error) {
	if err := f.call("describe"); err != nil {
		return Volume{}, err
	}
	return Volume{ID: "vol-source", SizeGiB: 100, AvailabilityZone: "ap-northeast-1a", Type: "gp3", Encrypted: true}, nil
}

func (f *fakeAPI) CreateVolume(_ context.Context, _ Volume, _ int64, clientToken string) (string, error) {
...
func (f *fakeAPI) AttachVolume(_ context.Context, volume, _ string, device string) error {
	return f.call("attach:" + volume + ":" + device)
}

func (f *fakeAPI) DetachVolume(_ context.Context, volume, _ string) error {
	return f.call("detach:" + volume)
}
...
func TestReplacementPersistsCreatedVolumeBeforeReadinessWait(t *testing.T) {
	...
	if op.TargetVolumeID != "vol-target" || op.Phase != PhaseRecoveryRequired {
...
func TestReplacementFailsBeforeMutationWhenPreflightOrTargetSizeUnsafe(t *testing.T) {
	...
	if len(api.calls) != 1 || api.calls[0] != "describe" {
...
func TestFileJournalPersistsPrivateAtomicState(t *testing.T) {
	...
	op := Operation{Version: 1, ID: "replace-1", Phase: PhaseVerified}
	if err := journal.Save(context.Background(), op); err != nil {
		t.Fatal(err)
	}
	info, err := os.Stat(filepath.Join(root, "replace-1.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(root, "replace-1.json")); !os.IsNotExist(err) {
