...
`push` は従来どおり GitHub repository、target ref、source commit、force-push の意味を capability service が評価した後にだけ実行します。SSH remote の場合は従来どおり Host の default key / `SSH_AUTH_SOCK` を利用できます。
