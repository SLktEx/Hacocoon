...
const (
	...
	brokerSocketReadyTimeout = time.Second
	brokerSocketReadyInterval = 10 * time.Millisecond
)
...
func (b *Broker) Repair(ctx context.Context, name string) error {
	...
	return waitForLocalBrokerSocket(ctx, b.socket(name))
}

// waitForLocalBrokerSocket makes repair completion deterministic across Go
// versions without widening authority: it probes only the local Unix listener.
func waitForLocalBrokerSocket(ctx context.Context, path string) error {
	deadline := time.NewTimer(brokerSocketReadyTimeout)
	defer deadline.Stop()
	ticker := time.NewTicker(brokerSocketReadyInterval)
	defer ticker.Stop()
	for {
		healthy, replaceable, err := localBrokerSocketState(path)
		if healthy {
			return nil
		}
		if err != nil {
