...
func validateConfiguration(configuration Configuration) ([]Area, error) {
	...
	for i := range areas {
		...
		if !cleanPath(a.Path) || a.Repository == "" && !strings.HasPrefix(a.Path, "/") || a.Repository != "" && (strings.HasPrefix(a.Path, "/") || a.Path == ".." || strings.HasPrefix(a.Path, "../") || !gitrepo.ValidID(a.Repository)) {
			return nil, core.ErrInvalidArgument
		}
...
func (s *Selector) repositoryPaths(work core.Workspace) (map[string]string, error) {
	...
	if !gitrepo.ValidID(id) || s.repositories == nil {
	...
	for _, member := range object.Copies() {
		if !gitrepo.ValidID(member.Repository) || paths[member.Repository] != "" || member.State != "ready" || member.Kind != "work" {
			return nil, core.ErrIncompatibleState
		}
		target := "/workspace"
