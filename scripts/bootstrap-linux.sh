...
if ! command -v apt-get >/dev/null 2>&1; then
  printf 'haco bootstrap: automatic Linux host dependency setup currently supports apt-based distributions only\n' >&2
  exit 1
fi
...
recovery_hint() {
  if [ "$IS_WSL" = "1" ]; then
    ...
    printf 'haco bootstrap: recover on the Physical Host with a root shell or sudo.\n' >&2
  fi
}
...
if [ "$pid1" != "systemd" ]; then
  ...
  printf 'haco bootstrap: systemd must be active as PID 1 on the Linux Physical Host\n' >&2
  exit 1
fi
...
if [ "$SKIP_INCUS" != "1" ]; then
  ...
  configure_hacocoon_controller "$controller_bin" || {
    printf 'haco bootstrap: failed to prepare Physical Host controller\n' >&2
    recovery_hint
    exit 1
  }
  ...
  $SUDO "$haco_bin" host ensure || {
    printf 'haco bootstrap: failed to prepare haco-host\n' >&2
    recovery_hint
    exit 1
  }
  ...
  if [ "$IS_WSL" = "1" ]; then
    ...
    printf '%s\n' 'haco bootstrap: native Linux login shell is unchanged; use haco host shell explicitly when entering the trusted Host.'
  fi
else
  if [ "$IS_WSL" = "1" ]; then
    printf '%s\n' 'haco bootstrap: -SkipIncus leaves the Physical Host login unchanged; haco-host auto-entry requires a ready Incus backend.'
  else
    printf '%s\n' 'haco bootstrap: -SkipIncus skips trusted haco-host reconciliation.'
  fi
fi

if [ "$SKIP_INCUS" != "1" ] && [ "$GRANT_INCUS_ADMIN" = "1" ] && [ "$(id -u)" -ne 0 ]; then
  printf '%s\n' 'haco bootstrap: start a new login session (or use newgrp incus-admin) before relying on the new group membership.'
fi