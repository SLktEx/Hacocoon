...
def owned_asset(data, name, require_unused=True):
    if data.get("version") not in (7, 8, 10, 11, 12, 13) or (require_unused and (data.get("environments") or data.get("workspace_leases") or data.get("snapshots") or data.get("restores") or data.get("snapshot_workspace_copies"))):
