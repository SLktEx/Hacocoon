...
class NetworkOutputTest(unittest.TestCase):
    ...
    def test_japanese_diagnostics_keep_safe_categories(self):
        self.assertEqual(command_failure_category("SECRET", "haco: セットアップ用の通常ファイルを読み取れません。SECRET"), "script-input")
        self.assertEqual(command_failure_category("SECRET", "haco: ログ設定が無効です。SECRET"), "logging-config")

    def test_missing_or_extra_results_fail(self):
        for output in ("PENDING_NETWORK_RESULT_OK\n", "Project setup completed.\n",
                       "プロジェクトのセットアップが完了しました。\n",
                       "PENDING_NETWORK_RESULT_OK\nプロジェクトのセットアップに失敗しました。\n",
                       "PENDING_NETWORK_RESULT_OK\nSECRET\nプロジェクトのセットアップが完了しました。\n",
                       "PENDING_NETWORK_RESULT_OK\nunexpected\nProject setup completed.\n"):
