...
class CleanupTests(unittest.TestCase):
    def check_case(self, mode, expected, deleted):
...
''')
            ...
            result = subprocess.run(["bash", "-c", 'set -euo pipefail; source "$1"; ci_delete_project "$2"', "test", str(LIBRARY), "hacocoon"], env=env, capture_output=True)
