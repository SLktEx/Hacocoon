#!/usr/bin/env python3
"""Drive Windows install/reinstall through the real user action path.

The branch-built release-equivalent ZIP is prepared before this harness starts.
From the first packaged BAT invocation onward, every product-driving action is a
command a real user can type unchanged. The harness may automate keystrokes and
observe output, but it must not inject Hacocoon test environment variables,
installer/E2E-only arguments or options, prepare/repair state through root WSL,
or replace a user action with a CI shortcut.
...
type only the ordinary Hacocoon commands declared below.
"""
...
def run_host_session(session: str, *, expected_output: list[str]) -> str:
    ...
    # so a CI shortcut cannot silently enter the product-driving path.
    try:
...
def main() -> int:
    ...
    require_output(second, r"Hacocoon WSL installation complete", phase="install completion")
    ...
    require_output(third, r"Hacocoon WSL installation complete", phase="reinstall")
    ...
    print("windows installer exact user action path: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"windows installer exact user action path: FAIL: {exc}", file=sys.stderr)
        raise
