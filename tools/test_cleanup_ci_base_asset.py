...
class CleanupTest(unittest.TestCase):
    ...
    def test_exact_cleanup_and_refusal(self):
        for mode in ("ok", "schema8", "restore", "absent", "lease", "foreign-owner", "host-device", "running", "duplicate", "still-present", "failed-delete"):
            with self.subTest(mode=mode):
                ...
                if mode in ("ok", "schema8", "absent"): cleanup(data, name, run)
                else:
                    with self.assertRaises(RuntimeError): cleanup(data, name, run)
                deletes = sum(c[0] == "delete" for c in calls)
                self.assertEqual(deletes, int(mode in ("ok", "schema8", "still-present", "failed-delete")))
