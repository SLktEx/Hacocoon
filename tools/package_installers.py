...
def windows_binary(archive: Path, component: str, binary: str) -> bytes:
    label = {"review": "review", "wsl": "WSL helper", "tunnel": "tunnel"}[component]
    native = archive.parent / archive.name.replace("haco_linux_", f"haco_{component}_windows_").replace(".tar.gz", ".zip")
    expected = archive_checksum_line(archive.parent / "checksums.txt", native.name).split()[0]
    if sha256(native) != expected:
        raise ValueError(f"Windows {label} archive checksum mismatch")
    with zipfile.ZipFile(native) as source:
        entries = [entry for entry in source.infolist() if entry.filename == binary]
        if len(entries) != 1 or not 0 < entries[0].file_size <= 64 * 1024 * 1024:
            raise ValueError(f"Windows {component} executable missing or invalid")
        return source.read(entries[0])


def package_windows(output: Path, archive: Path, checksum_line: str, version: str) -> None:
    binaries = {name: windows_binary(archive, component, name) for component, name in
                (("review", "haco-review.exe"), ("wsl", "haco-wsl.exe"), ("tunnel", "haco-tunnel.exe"))}
    for name, data in binaries.items():
        checksum_line += hashlib.sha256(data).hexdigest() + "  " + name + "\n"

    temporary = output.with_suffix(output.suffix + ".tmp")
    temporary.unlink(missing_ok=True)
    with zipfile.ZipFile(temporary, "w") as zf:
        add_zip_file(zf, ROOT / "install" / "install-windows.bat", "install-windows.bat", 0o644)
        add_zip_file(zf, ROOT / "install" / "install-windows.ps1", "install-windows.ps1", 0o644)
        add_zip_file(zf, ROOT / "install" / "windows-review.ps1", "windows-review.ps1", 0o644)
        for name, data in binaries.items():
            add_zip_bytes(zf, data, name, 0o755)
        add_zip_file(zf, ROOT / "install" / "install.sh", "install.sh", 0o755)
        add_zip_file(zf, ROOT / "install" / "incus-lts.sh", "incus-lts.sh", 0o755)
        add_zip_file(zf, ROOT / "install" / "setup-wsl-host-interop.py", "setup-wsl-host-interop.py", 0o755)
        add_zip_file(zf, ROOT / "internal/adapters/incus/packaging/incus-boot-guard.py", "incus-boot-guard.py", 0o755)
    ...
    with temporary.open("wb") as raw:
        with gzip.GzipFile(filename="", mode="wb", fileobj=raw, mtime=0) as gz:
            with tarfile.open(fileobj=gz, mode="w") as tf:
                add_tar_bytes(tf, (ROOT / "install" / "install-ubuntu.sh").read_bytes(), "install-ubuntu.sh", 0o755)
                add_tar_bytes(tf, (ROOT / "install" / "install.sh").read_bytes(), "install.sh", 0o755)
                add_tar_bytes(tf, (ROOT / "install" / "incus-lts.sh").read_bytes(), "incus-lts.sh", 0o755)
                add_tar_bytes(tf, (ROOT / "install" / "setup-wsl-host-interop.py").read_bytes(), "setup-wsl-host-interop.py", 0o755)
                add_tar_bytes(tf, (ROOT / "internal/adapters/incus/packaging/incus-boot-guard.py").read_bytes(), "incus-boot-guard.py", 0o755)
