...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $help -notmatch '(?m)--name\b') {
    ...
    Invoke-WslChecked @("--install", "Ubuntu-26.04", "--name", $Distro, "--no-launch")
    $Created = $true
    ...
    $pid1 = (& wsl.exe --distribution $Distro --user root -- sh -lc "ps -p 1 -o comm=").Trim()
    if ($LASTEXITCODE -ne 0 -or $pid1 -ne "systemd") {
        throw "systemd is not PID 1 inside the GitHub-hosted WSL2 distro (got '$pid1')"
    }

    $version = (& wsl.exe --distribution $Distro --user root -- sh -lc ". /etc/os-release; printf '%s' \"`$VERSION_ID\"").Trim()
    if ($LASTEXITCODE -ne 0 -or $version -ne "26.04") {
        throw "expected Ubuntu 26.04 inside WSL, got '$version'"
    }

    $LinuxRepo = (& wsl.exe --distribution $Distro --user root -- wslpath -u -a $Repo).Trim()
