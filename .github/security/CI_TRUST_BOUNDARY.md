...
The Hacocoon Core lifecycle E2E in `tools/ci-incus-core.sh` is a second reviewed exception. It runs independently on a fresh disposable Ubuntu 26.04 runner using the same signed Incus LTS bootstrap as standalone acceptance. It does not inherit another job's privileged state. Bootstrap and bounded daemon readiness are separate infrastructure steps; Core, egress and shipped controller/CLI lifecycle assertions are product steps. The runner-local TLS client lets Hacocoon run as the ordinary runner user, subordinate-ID mappings grant only the leased Workspace owner's UID/GID, and bridge filtering remains enabled. Reviewed helpers enable only explicitly selected native tests and require their PASS receipts. Cleanup accepts only `haco-e2e-*` or the exact `hacocoon` project with expected `haco-*` instances, and requires successful inventory plus positive absence. No repository secrets, write token, self-hosted runner, VM/KVM path or EC2 authority is exposed. The [PR contract inventory](../../docs/reliability/ci-contracts.md) owns routing, test coverage and retained evidence; privileged fixture permissions remain subject to this security boundary.

The split Incus storage jobs in `.github/workflows/incus-core-e2e.yml` (`incus-storage-cli`, `incus-host-tooling`, `incus-snapshots`, `incus-volume-base-build`, `incus-transfer`, `incus-evacuation-recovery` and `incus-maintenance`) are another narrow exception. Each job runs independently on a fresh disposable Ubuntu 26.04 runner, prepares the same reviewed real-Incus substrate, and executes the actual `haco` binary as the ordinary runner user before its focused native storage scenario. `tools/ci-incus-storage-cli.sh` verifies that normal `haco create` lazily creates and uses the real `haco-local-default` Incus-owned loop-backed Btrfs pool, `haco exec` sees a writable leased Workspace, `haco delete` removes the named Environment, and subsequent `haco run` reuses the pool and cleans up its ephemeral Environment. The jobs do not share privileged Incus state or runner-local storage with one another. Storage backing-image, loop, filesystem, and mount lifecycle remain owned by Incus. Failure cleanup accepts only the exact `hacocoon` project and expected Hacocoon resources; unexpected identities fail closed instead of being force-removed. No repository secrets, write token, self-hosted runner, arbitrary command execution, or persistent root authority is provided to Hacocoon.
