...
func TestDesktopChoiceTerminalProcess(t *testing.T) {
	if language := os.Getenv("HACO_TEST_CHOICE_CHILD"); language != "" {
...
env = os.environ.copy()
env.update(HACO_TEST_CHOICE_CHILD=sys.argv[5], HACO_CONTROL_SOCKET=sys.argv[2], HOME=sys.argv[3], WSL_INTEROP='', WSL_DISTRO_NAME='')
...
try:
    while time.monotonic() < deadline:
        if select.select([master], [], [], .1)[0]:
            ...
            if not sent and (b'Choose an Environment' in output or '環境を選択'.encode() in output):
    ...
    os.close(master)
`
					command := exec.Command(python, "-I", "-c", script, os.Args[0], socket, t.TempDir(), answer, language)
					output, err := command.CombinedOutput()
					if err != nil {
						t.Fatalf("terminal choice: %v %s", err, output)
					}
					if language == "ja" && !strings.Contains(string(output), "開く環境を選択") {
						t.Fatal("Japanese prompt missing", string(output))
					}
					ready := "SSH ready:"
					if language == "ja" {
						ready = "SSH接続を準備しました:"
					...
					if answer == "\n" {
						if access.Load() != 0 || strings.Contains(string(output), ready) {
							t.Fatal("cancel prepared access")
						}
					} else if access.Load() != 1 || !strings.Contains(string(output), ready+" haco-zeta") {
