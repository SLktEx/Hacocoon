...
func temporaryCommandWithInput(ctx context.Context, args []string, stdin io.Reader, out, diagnostic io.Writer) int {
	...
	if interactive && *asJSON {
		_, _ = fmt.Fprintln(diagnostic, cliMessage("run.stream_json"))
		return 2
	}
	if tty {
		input, ok := stdin.(interface{ Fd() uintptr })
		if !ok || !term.IsTerminal(int(input.Fd())) {
			_, _ = fmt.Fprintln(diagnostic, cliMessage("run.tty_required"))
			return 2
		}
	}
	...
	if ctx.Err() != nil {
		_, _ = fmt.Fprintln(diagnostic, cliMessage("run.canceled"))
		return 130
	}
	if *asJSON {
		...
		if result.Execution.StdoutTruncated || result.Execution.StderrTruncated {
			if _, err := fmt.Fprintln(diagnostic, cliMessage("run.truncated")); err != nil {
				return 1
			}
		}
	}
	if !result.CleanedUp {
		if result.Environment != "" {
			...
			_, _ = fmt.Fprintln(diagnostic, cliMessage("run.no_result"))
		}
		return 1
	}
	code := result.Execution.ExitCode
	if code < 0 || code > 255 {
		_, _ = fmt.Fprintln(diagnostic, cliMessage("run.invalid_exit"))
		return 1
	}
	if runErr != nil {
