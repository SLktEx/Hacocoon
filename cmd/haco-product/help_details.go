...
func init() {
	...
	set([]string{"run"}, []cliui.HelpField{field("-- <command...>", "detail.command")}, []cliui.HelpField{field("--workspace <workspace>", "run.flag_workspace"), base, noOCI, field("--read-only", "run.flag_readonly"), json, field("--rm", "run.flag_rm")}, "help.run")
	for i := range helpPages {
		page := &helpPages[i]
