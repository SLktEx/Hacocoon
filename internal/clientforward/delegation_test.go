...
func testRealCompanionCancellation(t *testing.T, command func(context.Context, string, ...string) *exec.Cmd, interrupt func(context.CancelFunc)) {
	...
	application.(*net.TCPListener).SetDeadline(time.Now().Add(8 * time.Second))
	...
	defer func() { application.Close(); <-applicationDone }()
	...
	defer active.Close()
	active.SetDeadline(time.Now().Add(6 * time.Second))
