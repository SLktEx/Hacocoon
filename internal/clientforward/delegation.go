...
func RunDelegated(ctx context.Context, input io.ReadCloser, out, diagnostic io.Writer, connect func(reclamation.WSLTarget) (*controlapi.Client, error)) int {
	...
	defer stopClose()
	timer := time.AfterFunc(10*time.Second, func() { _ = input.Close() })
	request, err := readDelegation(input)
	if !timer.Stop() || err != nil || ctx.Err() != nil {
		_, _ = fmt.Fprintln(diagnostic, cliui.English.Format("forward.delegate_invalid"))
		return 2
	}
	...
	_ = input.Close()
	if <-watch {
		_, _ = fmt.Fprintln(diagnostic, request.Request.Language.Format("forward.delegate_invalid"))
		return 1
	}
	return code
}
...
func runInstalled(ctx context.Context, request delegation, out, diagnostic io.Writer, connect func(reclamation.WSLTarget) (*controlapi.Client, error)) int {
	...
	if err != nil {
		...
		_, _ = fmt.Fprintln(diagnostic, message("forward.windows_unavailable"))
		return 1
	}
