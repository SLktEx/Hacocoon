...
func TestCapabilityFlowAcrossFilePolicyApprovalAndAudit(t *testing.T) {
	...
	policy := `{"default":"deny","rules":[{"capability":"local.echo","action":"echo","resource":"sensitive","parameters":{"message":"*"},"decision":"require-approval","reason":"sensitive local action"}]}`
	...
	if err != nil || result.Output != "private-parameter" || result.ExecutionState != core.CapabilitySucceeded || !result.AuditComplete || result.RequestID == "" {
		t.Fatalf("result=%#v err=%v", result, err)
	}
	if !strings.Contains(prompt.String(), "Approve capability local.echo") || !strings.Contains(prompt.String(), `message="private-parameter"`) {
