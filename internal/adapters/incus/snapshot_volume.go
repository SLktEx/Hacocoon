...
func (p snapshotVolumePlan) validate() error {
	...
	switch p.SourceKind {
	case "work":
		if !strings.HasPrefix(p.Source, "haco-work-") || !gitadapter.ValidID(strings.TrimPrefix(p.Source, "haco-work-")) || !gitadapter.ValidID(p.SourceID) || !strings.HasPrefix(p.Role, "workspace:") || !gitadapter.ValidID(strings.TrimPrefix(p.Role, "workspace:")) {
			return core.ErrInvalidArgument
		}
	case OCIStoreKind:
