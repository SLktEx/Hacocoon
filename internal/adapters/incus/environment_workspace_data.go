...
func (p *SandboxProvider) environmentPlacementBinding(ctx context.Context, request core.EnvironmentResourceBinding) (string, []WorkspaceAttachment, error) {
	...
	if request.ReadOnly || !managed || !gitadapter.ValidID(workID) || p == nil || p.BaseProvider == nil || p.Runtime == nil || p.managedWorkspace == nil {
...
func environmentWorkspaceObject(m WorkspaceAttachment) (gitrepo.Object, error) {
	...
	if !managed || !validWorkspaceAttachment(m) || !gitadapter.ValidID(m.Repository) || err != nil || len(owner) != 16 {
