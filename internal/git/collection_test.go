...
func TestTwoRepositoriesPushToTheirOwnRemotes(t *testing.T) {
	...
	for _, id := range []string{"one", "two"} {
		...
		if _, err := backend.RunGit(context.Background(), gitadapter.AgentRequest{Operation: "workspace", Repository: id, Workspace: member.ID, Remote: repo.Remote, Branch: "main"}); err != nil {
			t.Fatal(err)
		}
	...
	if _, err := gitadapter.UnixExchange(broker.socket("dev"))(ctx, gitadapter.Request{Operation: "list", Repository: "foreign"}); err == nil {
