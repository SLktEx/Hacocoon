...
func (b *recoveryBackend) RunGit(ctx context.Context, req gitadapter.AgentRequest) (gitadapter.Response, error) {
	...
	if req.Operation == "push" && b.loseAck && err == nil {
		return gitadapter.Response{}, errors.New("injected lost response after actual remote write")
	}
	return result, err
}
...
func (f *recoveryFixture) push(ctx context.Context) error {
	_, err := f.broker.perform(ctx, f.bound, f.proposal, func(ctx context.Context) (gitadapter.Response, error) {
...
func TestPushRecoveryCompetingIdenticalCreationIsStillUnconfirmed(t *testing.T) {
	...
	f.agent.OldOID, f.proposal.OldOID = gitadapter.ZeroOID, gitadapter.ZeroOID
...
func TestPushRecoveryRejectsTamperedAndPartialHistory(t *testing.T) {
	for _, kind := range []string{"owner", "oid", "duplicate", "partial"} {
		t.Run(kind, func(t *testing.T) {
			...
			case "oid":
				event.Attributes["new_oid"] = gitadapter.ZeroOID
			case "partial":
