...
func testGit(t *testing.T, dir string, args ...string) string {
	t.Helper()
	// Native fixture Git must finish maintenance before temporary repo cleanup.
...
func TestGitHelperProcess(t *testing.T) {
	...
	err := gitadapter.Helper(context.Background(), args[1:], os.Stdin, os.Stdout, os.Stderr, gitadapter.UnixExchange(os.Getenv("HACO_GIT_TEST_SOCKET")))
	if err != nil {
...
func ordinaryGitWorkflow(t *testing.T, largeBytes int64) {
	...
	if _, err := backend.RunGit(context.Background(), gitadapter.AgentRequest{Operation: "clone", Repository: repo.ID, Remote: repo.Remote, Branch: repo.Branch}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := backend.RunGit(context.Background(), gitadapter.AgentRequest{Operation: "workspace", Repository: repo.ID, Workspace: work.ID, Remote: repo.Remote, Branch: repo.Branch}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := gitadapter.UnixExchange(broker.socket("dev"))(ctx, gitadapter.Request{Operation: "list", Repository: "demo"}); err == nil {
	...
	for _, request := range []gitadapter.Request{{Operation: "list", Repository: "demo"}, {Operation: "fetch", Repository: "demo", Heads: []gitadapter.Head{{Ref: "refs/heads/feature/topic.v2", OID: feature}}}} {
		if _, err := gitadapter.UnixExchange(broker.socket("dev"))(ctx, request); err == nil {
			t.Fatal("broad reads bypassed exact-ref deny")
		}
	}
	probe := exec.CommandContext(ctx, "/usr/bin/git", "-C", filepath.Join(backend.repos, "demo"), "cat-file", "-e", feature)
	probe.Env = []string{"PATH=/usr/bin:/bin", "GIT_CONFIG_NOSYSTEM=1", "GIT_CONFIG_GLOBAL=/dev/null", "GIT_NO_REPLACE_OBJECTS=1"}
