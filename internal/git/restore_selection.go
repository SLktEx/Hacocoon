...
func ValidateRepositorySelection(repositories []string) error {
	...
	for _, name := range repositories {
		if !gitadapter.ValidID(name) || seen[name] {
			return core.ErrInvalidArgument
		}
		seen[name] = true
	}
...
func (s *RepositoryService) restoreSources(ctx context.Context, backend savedWorkspaceBackend, saved core.Snapshot, repositories []string) ([]restoreSource, error) {
	...
	for _, source := range all {
		if _, exists := byName[source.Repository]; exists || !gitadapter.ValidID(source.Repository) || !gitadapter.ValidWorkspaceRouting(source.Remote, source.Branch) || source.Component.State != "verified" {
			return nil, core.ErrIncompatibleState
		}
