...
func (b *Broker) beginPush(ctx context.Context, bound binding, request core.CapabilityRequest) (core.CapabilityAuditEvent, error) {
	...
	if b.PushAudit == nil || event.RequestID == "" || !core.ValidEnvironmentInstanceID(event.EnvironmentInstance) || !validPushAudit(event) || !gitadapter.ValidID(event.Attributes["repository_owner"]) {
...
func validPushAudit(event core.CapabilityAuditEvent) bool {
	...
	return event.RequestID != "" && event.Capability == Capability && event.Action == "push" &&
		gitadapter.ValidID(event.Environment) && gitadapter.ValidID(a["repository"]) && gitadapter.ValidID(a["operation_id"]) &&
		gitadapter.ValidateRemote(event.Resource) == nil && a["remote"] == event.Resource &&
		gitadapter.ValidHeadRef(a["target_ref"]) && gitadapter.ValidOID(a["old_oid"]) && gitadapter.ValidOID(a["new_oid"]) && a["new_oid"] != gitadapter.ZeroOID
}
...
func (b *Broker) pushStatus(ctx context.Context, environment, requestID string) (PushStatus, core.CapabilityAuditEvent, error) {
	...
	if !gitadapter.ValidID(environment) || len(requestID) > 256 {
	...
	_, err := b.AuditHistory.StreamAudit(ctx, 0, func(event core.CapabilityAuditEvent, _ int64) error {
		...
		switch event.Type {
		case pushStarted:
			if status.Dispatched || status.Completed != nil || !gitadapter.ValidID(event.Attributes["repository_owner"]) || !core.ValidEnvironmentInstanceID(event.EnvironmentInstance) {
				return core.ErrRecoveryRequired
			}
...
func (b *Broker) ReconcilePush(ctx context.Context, environment, requestID string) (PushStatus, error) {
	...
	_, err = b.perform(ctx, bound, proposal, func(ctx context.Context) (gitadapter.Response, error) {
