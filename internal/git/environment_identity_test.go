...
func TestGitPreparedOperationRefusesReplacedEnvironmentIdentity(t *testing.T) {
	...
	executed := false
	_, err := broker.perform(context.Background(), binding{Environment: environments.environment}, Proposal{Environment: "dev", Repository: "repo", Remote: "https://example.com/owner/repo.git", Ref: "refs/heads/main", Operation: "push"}, func(context.Context) (gitadapter.Response, error) { executed = true; return gitadapter.Response{}, nil })
