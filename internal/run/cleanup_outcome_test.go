	...
	for _, reconcile := range []bool{false, true} {
		for _, markerFailure := range []bool{false, true} {
			...
			t.Run(fmtOutcome(reconcile, markerFailure), func(t *testing.T) {
				...
				var err error
				if reconcile {
					instance, identityErr := core.NewEnvironmentInstanceID()
					if identityErr != nil {
						t.Fatal(identityErr)
					}
					store.runs[name] = core.EphemeralRun{InstanceID: instance, EnvironmentID: name, State: core.EphemeralRunActive, CreatedAt: time.Now().UTC()}
