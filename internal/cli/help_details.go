...
func init() {
	...
	set([]string{"doctor"}, []cliui.HelpField{field("[environment]", "detail.doctor_env")}, []cliui.HelpField{json, field("--fix", "env.doctor.fix")})
	...
	set([]string{"env start", "env stop", "env delete", "env ssh-config"}, []cliui.HelpField{env}, nil, "detail.retention")
	...
	set([]string{"env forward"}, []cliui.HelpField{env}, []cliui.HelpField{field("--target-port <port>", "detail.target_port"), protocol, field("--port <local-port>", "detail.local_port")})
	...
	set([]string{"base import"}, []cliui.HelpField{field("<image.tar>", "base.import.archive")}, []cliui.HelpField{field("--name <base>", "base.packer_name"), json})
	set([]string{"base build"}, []cliui.HelpField{field("<directory | definition.json>", "base.packer_context")}, []cliui.HelpField{field("--name <base>", "base.packer_name"), field("--from <base>", "base.packer_from"), field("--builder <env>", "base.builder_name"), field("--output", "base.packer_output"), json})
	...
	for i := range helpPages {
		page := &helpPages[i]
		if !strings.Contains(page.Syntax, "--json") {
			continue
		}
		found := false
		...
		if !found {
			page.Options = append(page.Options, json)
		}
	}
