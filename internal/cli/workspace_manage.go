...
func managedWorkspaceCommand(ctx context.Context, args []string, in io.Reader, out, diagnostic io.Writer) int {
	...
	pos := flags.Args()
	if (args[0] == "list" && len(pos) != 0) || (args[0] == "delete" && (len(pos) != 1 || !gitadapter.ValidID(pos[0]))) {
		flags.Usage()
		return 2
	}
	...
	for i := range all {
		if all[i].Name == pos[0] {
			if selected != nil {
				_, _ = fmt.Fprintln(diagnostic, cliMessage("workspace.duplicate"))
				return 1
			}
			selected = &all[i]
		}
	...
	if selected == nil {
		_, _ = fmt.Fprintln(diagnostic, cliMessage("workspace.missing"))
		return 1
	}
	...
	if len(selected.Environments) != 0 {
		_, _ = fmt.Fprintln(diagnostic, cliMessage("workspace.busy"))
		return 1
	}
	if selected.State != "ready" && selected.State != "deleting" {
		_, _ = fmt.Fprintln(diagnostic, cliMessage("workspace.incomplete"))
		return 1
	}
	...
	if err := client.DeleteManagedWorkspace(ctx, selected.Workspace); err != nil {
		_, _ = fmt.Fprintf(diagnostic, cliLanguage().Text("workspace.delete_failed"), err)
		return 1
	}
...
func writeManagedWorkspaces(out io.Writer, all []workspace.ManagedWorkspace) error {
	...
	_, _ = fmt.Fprintln(table, cliMessage("workspace.columns"))
	for _, w := range all {
