...
func TestInteractiveWindowsLanguageHonorsExplicitLocale(t *testing.T) {
	for _, variable := range []string{"LC_ALL", "LC_MESSAGES"} {
		t.Run(variable, func(t *testing.T) {
			env := map[string]string{"WSL_DISTRO_NAME": "Hacocoon", "LANG": "ja_JP.UTF-8", variable: "C"}
			got := hostEntryLanguage(context.Background(), func(k string) string { return env[k] }, func(context.Context) (string, error) {
				t.Fatal("Windows detection must not override an explicit locale")
				return "ja", nil
			})
			if got != cliui.English {
				t.Fatalf("explicit %s=C selected %s", variable, got)
			}
		})
	}
}
