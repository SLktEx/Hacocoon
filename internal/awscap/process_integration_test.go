...
func(allowPolicy)Evaluate(context.Context,core.CapabilityRequest)(core.PolicyEvaluation,error){return core.PolicyEvaluation{Decision:core.PolicyAllow},nil}
type audit struct{events []core.CapabilityAuditEvent}
func(a *audit)Record(_ context.Context,e core.CapabilityAuditEvent)error{a.events=append(a.events,e);return nil}

func TestAWSReadCapabilityCrossesRealProcessBoundaryWithoutCredentialArguments(t *testing.T){
	root:=t.TempDir();bin:=filepath.Join(root,"bin");if err:=os.Mkdir(bin,0o755);err!=nil{t.Fatal(err)};log:=filepath.Join(root,"aws.log")
	script:=`#!/bin/sh
set -eu
printf '%s\n' "$*" >> "$HACO_FAKE_AWS_LOG"
case " $* " in
