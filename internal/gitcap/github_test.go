...
func (s envStore) GetEnvironment(context.Context, string) (core.Environment, error) {
	return s.env, nil
}
...
func TestParseGitHubRemote(t *testing.T) {
	...
	for _, raw := range []string{
		...
		"ssh://root@github.com/acme/demo.git",
		"https://gitlab.com/acme/demo.git",
		"file:///tmp/demo.git",
		"https://github.com/acme/demo/extra.git",
		"https://github.com/acme/demo.git?token=secret",
	} {
...
func TestBrokerPushUsesApprovedExactSourceSHA(t *testing.T) {
	...
	runner := &runnerFunc{fn: func(_ string, args []string) (host.Result, error) {
		...
		case strings.Contains(joined, "cat-file -e "+shaA+"^{commit}"):
			return host.Result{}, nil
		case strings.Contains(joined, "push --porcelain origin "+shaA+":refs/heads/feature/x"):
	...
	audit := &auditSink{}
	caps := capabilityapp.New(allowPolicy{}, noApproval{}, audit, provider)
	broker := NewBroker(runner, store, caps)
	result, err := broker.Push(context.Background(), PushSpec{Environment: "demo", Branch: "feature/x"})
	if err != nil || result.Provider != GitHubCapability {
		t.Fatalf("result=%#v err=%v calls=%v", result, err, runner.calls)
	}
	last := runner.calls[len(runner.calls)-1]
	if !strings.Contains(last, shaA+":refs/heads/feature/x") || strings.Contains(last, "HEAD:refs/heads/feature/x") {
		t.Fatalf("push was not pinned to approved SHA: %s", last)
	}
	decision := audit.events[1]
	if decision.Resource != "github://acme/demo/refs/heads/feature/x" || decision.Attributes["source_sha"] != shaA || decision.Attributes["organization"] != "acme" || decision.Attributes["repository"] != "demo" {
...
func TestProviderRejectsRemoteMutationAfterPolicy(t *testing.T) {
	...
	provider := NewProvider(runner, store)
	caps := capabilityapp.New(allowPolicy{}, noApproval{}, &auditSink{}, provider)
	_, err := NewBroker(runner, store, caps).Push(context.Background(), PushSpec{Environment: "demo", Branch: "feature/x"})
