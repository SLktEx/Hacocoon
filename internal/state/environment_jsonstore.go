...
const environmentStateVersion = 11 // 9 was an unpublished replacement prototype; reject it.
...
func normalizeEnvironmentState(data *environmentFileState) error {
	...
	for id, a := range data.BaseAssets {
		if (data.Version != 7 && data.Version != 8 && data.Version != 10 && data.Version != environmentStateVersion) || id != a.ID || validateBaseAsset(a) != nil {
	...
	for id, snapshot := range data.Snapshots {
		...
		if (data.Version != 5 && data.Version != 6 && data.Version != 7 && data.Version != 8 && data.Version != 10 && data.Version != environmentStateVersion) || id != snapshot.ID || validateSnapshot(snapshot) != nil {
			return core.ErrIncompatibleState
		}
	}
	if data.Version == 8 {
	...
	for id, op := range data.Restores {
		if (data.Version != 8 && data.Version != 10 && data.Version != environmentStateVersion) || id != op.ID || validateRestore(op) != nil || !reflect.DeepEqual(data.Snapshots[op.Saved.ID], op.Saved) || (op.Before.ID != "" && !reflect.DeepEqual(data.Snapshots[op.Before.ID], op.Before)) {
			return core.ErrIncompatibleState
		}
		before := op.Current
