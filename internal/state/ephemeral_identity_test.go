...
func TestRunIdentityCannotBeReassignedDuringLifecycle(t *testing.T) {
	...
	for _, change := range []func(*core.EphemeralRun){
		...
		if err := s.PutEphemeralRun(ctx, changed); !errors.Is(err, want) {
...
func TestRunStateWithoutCreationIdentityIsRefusedWithoutMutation(t *testing.T) {
	s, run, lease := runIdentityFixture(t)
	run.InstanceID = ""
	lease.Ephemeral = false
	data := environmentFileState{Version: environmentStateVersion, EphemeralRuns: map[string]core.EphemeralRun{run.EnvironmentID: run}, Leases: map[string]core.WorkspaceLease{lease.EnvironmentID: lease}}
	raw, err := json.Marshal(data)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.ListEphemeralRuns(context.Background()); !errors.Is(err, core.ErrIncompatibleState) {
	...
	if string(actual) != string(raw) {
		t.Fatal("invalid ownership evidence changed")
	}
}
